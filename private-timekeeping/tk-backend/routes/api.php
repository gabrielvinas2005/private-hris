...
use App\Http\Controllers\ReportDocxController;
use App\Http\Controllers\ReportExcelController;
use App\Http\Controllers\ReportHtmlController;
...
Route::options('/reports/pdf', function () {
    return response('', 200)
        ->header('Access-Control-Allow-Origin', '*')
        ->header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS')
        ->header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With')
...
Route::options('/reports/pdf/preview', function (Request $request) {
...
// DOCX CORS preflight
Route::options('/reports/docx', function (Request $request) {
    $origin = $request->headers->get('Origin') ?? '*';
    return response('', 200)
        ->header('Access-Control-Allow-Origin', $origin)
        ->header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS')
...
// Excel CORS preflight
Route::options('/reports/excel', function (Request $request) {
    $origin = $request->headers->get('Origin') ?? '*';
    return response('', 200)
        ->header('Access-Control-Allow-Origin', $origin)
        ->header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS')
...
// HTML Preview CORS preflight
Route::options('/reports/html', function (Request $request) {
    $origin = $request->headers->get('Origin') ?? '*';
    return response('', 200)
        ->header('Access-Control-Allow-Origin', $origin)
        ->header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS')
...
// CORS test endpoint for preview
Route::get('/reports/pdf/preview/test', function () {
    return response()->json(['message' => 'PDF Preview CORS is working!', 'timestamp' => now()])
        ->header('Access-Control-Allow-Origin', '*')
        ->header('Access-Control-Allow-Methods', 'POST, GET, OPTIONS')
        ->header('Access-Control-Allow-Headers', 'Content-Type, Authorization, X-Requested-With')
        ->header('Access-Control-Allow-Credentials', 'true');
});
Route::options('/reports/pdf/preview/template', function (Request $request) {
    $origin = $request->headers->get('Origin', '*');
    return response('', 200)
        ->header('Access-Control-Allow-Origin', $origin)
        ->header('Access-Control-Allow-Methods', 'POST, OPTIONS')
...
Route::post('/reports/pdf', [ReportPdfController::class, 'generate']);
Route::post('/reports/pdf/template', [ReportPdfController::class, 'generateFromTemplate']);
Route::post('/reports/pdf/styles', [ReportPdfController::class, 'generateWithStyles']);
Route::post('/reports/pdf/preview', [ReportPdfController::class, 'generatePreview']);
Route::post('/reports/pdf/preview/template', [ReportPdfController::class, 'generatePreviewFromTemplate']);

// DOCX generation routes
Route::post('/reports/docx', [DailyTimeRecordController::class, 'exportDocxReport']);
Route::post('/reports/docx/attendance', [ReportDocxController::class, 'generate']);
Route::post('/reports/docx/shift-schedule', [ReportDocxController::class, 'generate']);
Route::post('/reports/docx/assigned-employees', [ReportDocxController::class, 'generate']);
Route::post('/reports/docx/shifting-schedule-details', [ReportDocxController::class, 'generate']);

// Excel generation routes
Route::post('/reports/excel', [DailyTimeRecordController::class, 'exportExcelReport']);
Route::post('/reports/excel/attendance', [ReportExcelController::class, 'generate']);
Route::post('/reports/excel/shift-schedule', [ReportExcelController::class, 'generate']);
Route::post('/reports/excel/assigned-employees', [ReportExcelController::class, 'generate']);
Route::post('/reports/excel/shifting-schedule-details', [ReportExcelController::class, 'generate']);

// HTML preview generation routes
Route::post('/reports/html', [ReportHtmlController::class, 'generate']);

Route::get('/vacancies', 'VacanciesController@index');

// In development, allow DevAuth middleware to auto-auth protected process endpoints
if (config('auth.dev_auth_enabled', false)) {
    Route::middleware([\App\Http\Middleware\DevAuth::class])->group(function () {
        // Overtime approve via DevAuth (duplicate route outside sanctum for dev convenience)
        Route::post('/overtime-applications/approve', 'OvertimeApplicationController@approve');
    });
}

// Image serving route with CORS headers
Route::get('/images/{path}', 'ImageController@serveImage')->where('path', '.*');
...
Route::match(['get', 'post'], '/dev-login', 'Api\AuthController@devLogin');
...
Route::middleware('auth:sanctum')->get('/user', function (Request $request) {
    ...
    if ($user->employee_no) {
        $employeeRecord = DB::table('employees')
            ->leftJoin('name_suffixes as ns', 'ns.id', '=', 'employees.name_suffix_id')
            ->select(
    ...
    if (!$formattedName && $employeeRecord) {
        $nameParts = array_filter([
            $employeeRecord->first_name ?? null,
            $employeeRecord->middle_name ?? null,
            $employeeRecord->last_name ?? null,
            $employeeRecord->suffix ?? null,
        ]);
        $formattedName = trim(implode(' ', $nameParts));
    }

    if (!$formattedName && $user->email) {
        $formattedName = strstr($user->email, '@', true) ?: $user->email;
    }
    ...
    return response()->json([
        ...
        'data' => [
            'user' => [
                ...
                'first_name' => $employeeRecord->first_name ?? null,
                'middle_name' => $employeeRecord->middle_name ?? null,
                'last_name' => $employeeRecord->last_name ?? null,
                'suffix' => $employeeRecord->suffix ?? null,
            ]
        ],
...
Route::middleware(['auth:sanctum'])->group(function () {
    ...
    Route::get('/overtime-applications/{id}', 'OvertimeApplicationController@show')->whereNumber('id');
    Route::post('/overtime-applications/store', 'OvertimeApplicationController@store');
    Route::post('/overtime-applications/approve', 'OvertimeApplicationController@approve');
    Route::post('/overtime-applications/{id}/convert-to-cto', 'OvertimeApplicationController@convertToCTO');
    Route::post('/overtime-applications/{id}/revert-cto', 'OvertimeApplicationController@revertCTO');
    ...
    Route::post('/overtime-authorization-docx', 'OvertimeApplicationController@downloadOvertimeAuthorizationDocx');
    Route::post('/overtime-authorization-excel', 'OvertimeApplicationController@downloadOvertimeAuthorizationExcel');
    ...
    Route::get('/official-business-pass-slip-docx/{id}', 'OfficialBusinessApplicationController@downloadOfficialBusinessDocx');
    Route::get('/official-business-pass-slip-excel/{id}', 'OfficialBusinessApplicationController@downloadOfficialBusinessExcel');
    Route::get('/travel-authority-personal-docx/{id}', 'OfficialBusinessApplicationController@downloadTravelAuthorityPersonalDocx');
    Route::get('/travel-authority-docx/{id}', 'OfficialBusinessApplicationController@downloadTravelAuthorityDocx');
    Route::get('/request-pickup-print/{id}', 'OfficialBusinessApplicationController@printRequestForPickup');
    Route::get('/request-pickup-docx/{id}', 'OfficialBusinessApplicationController@downloadRequestForPickupDocx');
    Route::get('/request-pickup-excel/{id}', 'OfficialBusinessApplicationController@downloadRequestForPickupExcel');
    Route::get('/travel-order-ldsd-docx/{id}', 'OfficialBusinessApplicationController@downloadTravelOrderLDSDDocx');
    ...
    Route::get('/leave-application-docx/{id}', 'LeaveController@downloadDocx');
    ...
    Route::get('/fix-schedules/{id}/delete', 'FixScheduleController@delete');
    Route::delete('/fix-schedules/{id}', 'FixScheduleController@destroy');
    Route::get('/fix-schedules/{id}/employees', 'FixScheduleController@employees');
    Route::post('/fix-schedules/{id}/employees', 'FixScheduleController@addEmployees');
    Route::delete('/fix-schedules/{id}/employees/{employee_id}', 'FixScheduleController@removeEmployee');
    Route::post('/fix-schedules/{id}/employees/{employee_id}/remove', 'FixScheduleController@removeEmployee');
    
    // Assign Schedule
    Route::get('/assign-schedules', 'AssignFixScheduleController@index');
    Route::get('/assign-schedules/employees', 'AssignFixScheduleController@employees');
    Route::post('/assign-schedules', 'AssignFixScheduleController@store');
    
    // Leave Credits
    Route::get('/leave-credits', 'LeaveCreditController@index');
    // List all leave credits with employee and leave type info
    Route::get('/leave-credits/all', 'LeaveCreditController@listAll');
    Route::post('/leave-credits', 'LeaveCreditController@store');
    // Leave Beginning Balances
    Route::get('/leave-beginning-balances', 'LeaveCreditController@listBeginningBalances');
    Route::post('/leave-beginning-balances', 'LeaveCreditController@storeBeginningBalance');
    Route::post('/leave-beginning-balances/bulk', 'LeaveCreditController@storeBeginningBalancesBulk');
    Route::delete('/leave-beginning-balances/{id}', 'LeaveCreditController@deleteBeginningBalance');
    ...
    // Add missing delete endpoints for shift schedules
    Route::delete('/shift-schedules/{id}', 'ShiftScheduleController@destroy');
    Route::get('/shift-schedules/{id}/delete', 'ShiftScheduleController@destroy');
    
    // Process Daily Time Records
    Route::post('/process-attendance', 'ProcessAttendanceController@process');
    Route::post('/process-attendance/reprocess-all', 'ProcessAttendanceController@reprocessAll');
    Route::get('/process-attendance/progress-latest', 'ProcessAttendanceController@getLatestProcessProgress');
    Route::get('/process-attendance', 'ProcessAttendanceController@index');
    Route::post('/process-attendance/sync-time-data', 'ProcessAttendanceController@syncTimeDataPayrollPeriod');
    Route::get('/process-attendance/periods/{interval_id}', 'ProcessAttendanceController@getPayrollPeriods');
    Route::get('/process-attendance/period-status/{payroll_period_id}', 'ProcessAttendanceController@checkPayrollPeriodStatus');
    Route::get('/process-attendance/employees/{payroll_period_id}', 'ProcessAttendanceController@getEmployeeAttendanceData');
    Route::get('/process-attendance/days-present/{payroll_period_id}', 'ProcessAttendanceController@daysPresentSummary');
    Route::get('/process-attendance/leave-reversal-preview/{payroll_period_id}', 'ProcessAttendanceController@getLeaveReversalPreview');
    Route::get('/process-attendance/offset-data/{payroll_period_id}', 'ProcessAttendanceController@getOffsetData');
    Route::get('/process-attendance/offset-employee-summary/{payroll_period_id}', 'ProcessAttendanceController@getOffsetEmployeeSummary');
    Route::get('/process-attendance/offset-details/{employee_id}/{payroll_period_id}', 'ProcessAttendanceController@getEmployeeOffsetDetails');
    Route::post('/process-attendance/apply-offset', 'ProcessAttendanceController@applyOffset');
    Route::post('/process-attendance/apply-offset-all', 'ProcessAttendanceController@applyOffsetAll');
    Route::post('/process-attendance/calculate-offset-total', 'ProcessAttendanceController@calculateOffsetTotalDays');
    Route::get('/process-attendance/{employee_id}/{payroll_period_id}/report', 'ProcessAttendanceController@print');
    Route::get('/process-attendance/{employee_id}/{payroll_period_id}/dtr', 'ProcessAttendanceController@viewDTR');
    Route::get('/process-attendance/{employee_id}/{payroll_period_id}/dtr/data', 'ProcessAttendanceController@getDTRData');
    ...
    Route::post('/process-attendance/cancel-offset-all', 'ProcessAttendanceController@cancelOffsetAll');
    ...
    // Pass Slip Monitoring
    Route::get('/pass-slip-monitoring', 'PassSlipController@monitoring');
    Route::get('/pass-slip-pdf/{id}', 'PassSlipController@generatePDF');
    Route::get('/pass-slip-docx/{id}', 'PassSlipController@generateDocx');
    ...
    Route::get('/work-cancellations/create', 'WorkCancellationController@create');
    Route::get('/work-cancellations/{id}/add', 'WorkCancellationController@add');
    Route::get('/work-cancellations/{id}/edit', 'WorkCancellationController@edit');
    Route::post('/work-cancellations/store/{id}', 'WorkCancellationController@store');
    Route::put('/work-cancellations/update/{id}', 'WorkCancellationController@update');
    Route::get('/work-cancellations/{id}', 'WorkCancellationController@show');
    Route::delete('/work-cancellations/destroy/{id}', 'WorkCancellationController@destroy');
    
    // COC Details
    Route::get('/coc-details', 'COCDetailsController@index');
    
    // Months reference data
    Route::get('/months', 'MonthsController@index');
    ...
    // Time Data Tardiness Reports
    Route::get('/time-data/tardiness-reports', 'TardinessReportsController@index');
    Route::get('/time-data/tardiness-export', 'TardinessReportsController@export');
    Route::get('/time-data/tardiness-summary', 'TardinessReportsController@summary');
    Route::get('/time-data/{id}', 'TardinessReportsController@show');
    Route::get('/time-data/employee-tardiness/{employeeId}', 'TardinessReportsController@employeeTardiness');
    ...
    Route::get('/leave-credit-card/leave-types', 'LeaveCreditCardController@getLeaveTypes');
    Route::post('/leave-credit-card/bulk-preview', 'LeaveCreditCardController@bulkPreview');
    ...
    Route::get('/biometrics/realtime', 'BiometricsController@realtime');
    Route::get('/biometrics/daily-summary', 'BiometricsController@dailySummary');
