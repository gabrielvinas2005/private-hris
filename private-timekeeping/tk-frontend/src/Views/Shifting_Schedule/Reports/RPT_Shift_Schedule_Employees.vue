...
const { generateEmployeeDetailsReport } = useUnifiedReport()

function sanitizeFilename(name) {
  return (name || '').replace(/[^a-zA-Z0-9]/g, '_') || 'assigned_employees'
}

const computedFilename = computed(() => 
  `assigned_employees_${sanitizeFilename(props.scheduleName)}_${new Date().toISOString().slice(0, 10)}`
)

const reportHtmlContent = computed(() => {
  if (!props.employees || !props.employees.length) return ''
  
  const report = generateEmployeeDetailsReport(
    props.employees,
    props.scheduleName
  )
...
async function onExportExcel() {
  try {
    ...
    const reportData = {
      report_type: 'shift_schedule_employees',
      data: {
