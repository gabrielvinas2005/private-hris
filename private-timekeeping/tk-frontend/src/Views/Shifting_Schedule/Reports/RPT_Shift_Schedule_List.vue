...
const { generateShiftScheduleListReport } = useUnifiedReport()

function sanitizeFilename(name) {
  return (name || '').replace(/[^a-zA-Z0-9]/g, '_') || 'shift_schedule'
}
...
const reportHtmlContent = computed(() => {
  ...
  const report = generateShiftScheduleListReport(props.items)
...
async function onExportExcel() {
  try {
    ...
    const reportData = {
      report_type: 'shift_schedule_list',
      data: {
...
async function onExportPDF() {
  const report = generateShiftScheduleListReport(
    props.items,
    { filename: computedFilename.value }
  )
