<template>
  <div class="yearend-bonus-list benefit-table-card">
    <!-- Disclaimer -->
    <div class="yearend-disclaimer">
    ...
    <div class="table-container benefit-table-container">
      ...
      >
        ...
        <el-table-column prop="photo" label="Photo" width="70" align="center">
        ...
        <el-table-column
          prop="cash_gift_amount"
          label="Cash Gift"
          width="110"
          align="left"
        >
          <template #default="scope">
            <div class="cash-gift-cell">
              <span
                class="cash-gift-amount"
                style="color: #8b5cf6; font-weight: 700"
                >₱{{ formatCurrency(scope.row.cash_gift_amount) }}</span
              >
            </div>
          </template>
        </el-table-column>
        ...
        >
          <template #default="scope">
            <div class="total-earned-cell">
              <span
                class="total-earned-amount"
                style="color: #ef4444; font-weight: 700"
                >₱{{ formatCurrency(getTotalAmount(scope.row)) }}</span
              >
            </div>
          </template>
        </el-table-column>
    ...
    <div v-if="!loading && filteredData.length === 0" class="empty-state">
      ...
      <h3>No Year-End Bonus Records Found</h3>
      <p>
    ...
    <div v-if="selectedRecords.length > 0" class="bulk-actions">
      <div class="bulk-actions-content">
        ...
        <div class="bulk-buttons">
          ...
          <el-button type="primary" @click="bulkGenerateReport">
            Generate Report
          </el-button>
      ...
      <div v-if="selectedRecord" class="details-content">
        ...
        <div class="detail-row">
          ...
          <span class="bonus-highlight detail-value-with-edit">
            ...
            />
            <span v-else>₱{{ formatCurrency(selectedRecord.bonus_amount) }}</span>
          </span>
        </div>
        <div class="detail-row">
          <label>Cash Gift:</label>
          <span class="cash-gift-highlight">
            <el-input-number
              v-if="detailsEditMode"
              v-model="editCashGiftAmount"
              :min="0"
              :precision="2"
              :controls="false"
              size="small"
              class="detail-amount-input"
            />
            <span v-else>₱{{ formatCurrency(selectedRecord.cash_gift_amount) }}</span>
          </span>
        </div>
        <div class="detail-row">
          ...
          <span class="total-highlight">
            ₱{{
              formatCurrency(
                detailsEditMode
                  ? (Number(editBonusAmount) || 0) + (Number(editCashGiftAmount) || 0)
                  : getTotalAmount(selectedRecord)
              )
            }}
          </span>
        </div>
        <div v-if="detailsEditMode" class="detail-row detail-actions">
          ...
          <span class="detail-action-buttons">
            <el-button size="small" @click="cancelDetailsEdit">Cancel</el-button>
...
const editCashGiftAmount = ref(0);
...
const formatServiceDuration = (daysPresent) => {
  ...
  return `${months} month${months !== 1 ? "s" : ""} ${days} day${days !== 1 ? "s" : ""} (which is ${d} days_present)`;
};

const getTotalAmount = (row) => {
  const bonus = Number(row.bonus_amount) || 0;
  const cash = Number(row.cash_gift_amount) || 0;
  return bonus + cash;
};
...
const startDetailsEdit = () => {
  ...
  editCashGiftAmount.value = Number(selectedRecord.value.cash_gift_amount) || 0;
...
const saveDetailsAmounts = async () => {
  ...
  emit("update", {
    ...
    cash_gift_amount: Number(editCashGiftAmount.value) || 0,
  });
  detailsEditMode.value = false;
...
const deleteRecord = async (record) => {
  try {
    await ElMessageBox.confirm(
      `Are you sure you want to delete the year-end bonus record for ${record.name}?`,
      "Confirm Delete",
      {
