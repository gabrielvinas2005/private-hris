...
class MonetizationController extends Controller
{
    ...
    public function addMonetizationPayroll($id)
    {
        ...
        $monetizationEmployeesQuery = DB::table('employees as a')
        ...
        PayrollBenefitsEmployeeScope::apply($monetizationEmployeesQuery, 'a');
        $monetization_employees = $monetizationEmployeesQuery
    ...
    public function storeMonetizationPayroll(Request $request, $id)
    {
        ...
        if ($monetization_exist > 0) {
            return $this->errorResponse('Existing Monetization Payroll with the same Month and Year!', 400);
        }
    ...
    public function deleteMonetizationPayroll($id)
    {
        try {
            // Fetch header and ensure it exists and is not posted
            $header = DB::table('monetization_payroll_headers')->where('id', $id)->first();

            if (!$header) {
                return $this->errorResponse('Monetization payroll not found.', 404);
            }

            if (!empty($header->posted)) {
                return $this->errorResponse('Cannot delete a posted monetization payroll. Please unpost it first.', 422);
            }

            // Reset related leave monetizations as not processed
            $details = DB::table('monetization_payroll_details')
                ->where('monetization_payroll_id', $id)
                ->get();

            foreach ($details as $dtl) {
                if (!empty($dtl->monetization_id)) {
                    DB::table('leave_monetizations')
                        ->where('id', $dtl->monetization_id)
                        ->update(['is_processed' => false]);
                }
            }

            // Delete detail rows then header
            DB::table('monetization_payroll_details')->where('monetization_payroll_id', $id)->delete();
            DB::table('monetization_payroll_headers')->where('id', $id)->delete();
            ...
            $data_audit = array(
                ...
                'menu'    => 'Monetization Payroll',
                'activity' => 'Delete',
                'description' => 'Deleted Monetization Payroll.',
            );

            Audit::create($data_audit);

            return $this->successResponse(['id' => $id], 'Monetization payroll deleted successfully');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to delete monetization payroll: ' . $e->getMessage());
        }
    }
    ...
    public function print(Request $request)
    {
        try {
            ...
            $monetization = DB::table('monetization_payroll_headers')
            ...
            // Use provided signatory names and positions directly from the request
            ...
            if ($signatory_exist->isEmpty()) {
                $signatoryId = DB::table('payroll_signatories')->insertGetId($signatories_data);
            } else {
                $signatoryId = $signatory_exist[0]->id;
                DB::table('payroll_signatories')
                    ->where('id', $signatoryId)
                    ->update($signatories_data);
            }
            ...
            // Load company logo image (same as other reports)
            ...
            $pdf = PDF::loadView(
                'monetizations.monetization_payroll_print',
                compact('monetization', 'company', 'signatories', 'image')
            )
