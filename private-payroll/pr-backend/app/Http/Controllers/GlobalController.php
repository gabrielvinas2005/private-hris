    ...
    public function getPayrollPeriod($id)
    {
        ...
        $data = DB::table('payroll_periods as a')
            ...
            return response()->json([
                'success' => false,
                'data' => [],
                'message' => 'Failed to retrieve payroll periods: ' . $e->getMessage()
            ], 500);
        }
    }
    ...
    public function getPayrollPeriodPosted($id)
    {
        try {
            $data = DB::table('payroll_periods as a')
                ...
                ->where('a.payroll_interval_id', $id)
                ->where('a.active', true)
                // Support historical storage variants: bit true, string 'true', or numeric 1
                ->where(function ($q) {
                    $q->where('a.posted', true)
                        ->orWhere('a.posted', 'true')
                        ->orWhere('a.posted', 1);
                })
                ->orderBy('a.release_date', 'desc')
                ->distinct()
            ...
            return response()->json([
                'success' => true,
                'data' => $data,
                'message' => 'Posted payroll periods retrieved successfully'
            ]);
        } catch (\Exception $e) {
            return response()->json([
                'success' => false,
                'data' => [],
                'message' => 'Failed to retrieve posted payroll periods: ' . $e->getMessage()
            ], 500);
        }
    }
