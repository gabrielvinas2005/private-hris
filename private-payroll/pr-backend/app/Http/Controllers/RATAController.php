...
class RATAController extends Controller
{
    ...
    public function loadRATAPayroll()
    {
        try {
            $rata = DB::table('rata_payroll_headers as a')
                ...
                ->leftJoin('rata_types as c', 'a.rata_type_id', '=', 'c.id')
                ->select(
                    'a.*',
                    'b.name as branch',
                    'c.name as rata_type_name',
                )
                ->get();
    ...
    public function addRATAPayroll($id)
    {
        try {
            ...
            $rata_types = DB::table('rata_types')
            ->where('is_active', 1)
            ...
            $rataEmployeesQuery = DB::table('employees as a')
            ...
            PayrollBenefitsEmployeeScope::apply($rataEmployeesQuery, 'a');
            $rata_employees = $rataEmployeesQuery->orderBy('a.first_name', 'asc')->get();

            return $this->successResponse([
                'data' => $data,
                'rata_types' => $rata_types,
    ...
    public function storeRATAPayroll(Request $request, $id)
    {
        try {
            ...
            // RATA payroll is not scoped by branch; column kept for legacy rows (stored as 0).
            $branchId = 0;

            $rata_exist = DB::table('rata_payroll_headers')
                ->where('rata_type_id', $request->rata_type_id)
            ...
            if ($rata_exist > 0) {
                return $this->errorResponse(
                    'A RATA payroll already exists for this type, month, and year.'
                );
            }
    ...
    public function deleteRATAPayroll($id)
    {
        try {
            DB::beginTransaction();

            DB::table('rata_payroll_details')->where('rata_id', $id)->delete();
            $deleted = DB::table('rata_payroll_headers')->where('id', $id)->delete();

            if ($deleted === 0) {
                DB::rollBack();
                return $this->errorResponse('RATA payroll record not found.', 404);
            }
            ...
            $data_audit = array(
                ...
                'menu'    => 'RATA Payroll',
                'activity' => 'Delete',
                'description' => 'Deleted RATA Payroll.',
            );

            Audit::create($data_audit);

            DB::commit();
            return $this->successResponse(['deleted' => $deleted], 'RATA payroll deleted successfully');
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->serverErrorResponse('Failed to delete RATA payroll: ' . $e->getMessage());
        }
    }
    ...
    public function rataReport()
    {
        try {
            ...
            $rata_payroll = DB::table('rata_payroll_headers as a')
                ->leftJoin('rata_types as rt', 'a.rata_type_id', '=', 'rt.id')
                ->select(
                    'a.id',
                    DB::raw(
                        "CONCAT(a.month, ' ', a.year_id, ' (', LTRIM(RTRIM(ISNULL(rt.name, 'RATA'))), ')') as name"
                    )
                )
                ->where('posted', true)
                ->get();
    ...
    public function print(Request $request)
    {
        try {
            ...
            // Fetch header and ensure it exists and is posted
            $rata_header = DB::table('rata_payroll_headers')
                ->where('id', $request->rata_payroll_id)
                ->first();

            if (!$rata_header) {
                return $this->errorResponse('Selected RATA payroll was not found.', 404);
            }

            if (empty($rata_header->posted)) {
                return $this->errorResponse('Selected RATA payroll is not yet posted.', 422);
            }
            ...
            $signatories = [
                ...
                'branch_id' => isset($rata_header->branch_id) ? $rata_header->branch_id : 0
            ];
            ...
            $period = trim(($rata_header->month ?? '') . ' ' . ($rata_header->year_id ?? ''));
            ...
            // Portrait letter matches typical general-payroll printouts; avoids overly wide landscape tabloid.
