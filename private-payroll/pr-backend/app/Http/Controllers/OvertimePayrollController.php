...
class OvertimePayrollController extends Controller
{
    ...
    {
        try {
            $payroll_intervals = DB::table('payroll_intervals as pi')
                ->join('payroll_periods as pp', 'pp.payroll_interval_id', '=', 'pi.id')
                ->where('pi.active', true)
                ->where('pp.active', true)
                // Load intervals that already have posted payroll periods.
                // Do not depend on overtime headers so users can create OT payroll for newly posted periods.
                ->where(function ($q) {
                    $q->where('pp.posted', true)
                        ->orWhere('pp.posted', 'true')
                        ->orWhere('pp.posted', 1);
                })
                ->select('pi.*')
                ->distinct()
    ...
    public function print(Request $request)
    {
        try {
            ...
            // Load selected payroll period window so OT rows are scoped correctly
            $period = DB::table('payroll_periods')
                ->select('attendance_start_date', 'attendance_end_date', 'release_date')
            ...
            $data = DB::table('overtime_payroll_headers as e')
                ...
                ->join('overtime_applications as f', function ($join) {
                    $join->on('d.employee_id', '=', 'f.employee_id');
                    $join->on('d.overtime_type_id', '=', 'f.overtime_type_id');
                })
                ...
                ->whereBetween('f.date', [$period->attendance_start_date, $period->attendance_end_date])
                ->groupBy(
                    'f.date',
                    'b.salary',
                    'c.rate',
           ...
           "),
                ...
                ->whereBetween('e.date', [$period->attendance_start_date, $period->attendance_end_date])
                ->groupBy([
                    'b.employee_id',
                    'c.photo',
                    'c.is_encrypted',
    ...
    public function load()
    {
        try {
            $overtime_payroll = DB::table('overtime_payroll_headers as a')
                ...
                ->leftJoin('overtime_payroll_details as opd', 'a.id', '=', 'opd.overtime_payroll_id')
                ->leftJoin('employees as e', 'opd.employee_id', '=', 'e.id')
                ->leftJoin('overtime_applications as oa', function ($join) {
                    $join->on('opd.employee_id', '=', 'oa.employee_id');
                    $join->on('opd.overtime_type_id', '=', 'oa.overtime_type_id');
                    $join->where('oa.payroll', true);
                    $join->where('oa.approved', true);
                    $join->where('oa.approved_2', true);
                })
                ->leftJoin('overtime_types as ot', 'oa.overtime_type_id', '=', 'ot.id')
                ->select(
                    'a.id',
                    ...
                    DB::raw("
                        SUM(
                            CASE
                                WHEN oa.id IS NOT NULL
                                     AND oa.date BETWEEN b.attendance_start_date AND b.attendance_end_date
                                THEN (((e.salary / 22) / 8) * oa.total_hours) * ot.rate
                                ELSE 0
                            END
                        ) +
                        ISNULL(
                            (
                                SELECT SUM(ISNULL(ts.Adjustment_Amount_OT_Holiday, 0))
                                FROM time_data_summary ts
                                WHERE ts.payroll_period_id = b.id
                                  AND ts.employee_id IN (
                                      SELECT DISTINCT opd2.employee_id
                                      FROM overtime_payroll_details opd2
                                      WHERE opd2.overtime_payroll_id = a.id
                                  )
                            ),
                            0
                        ) as total_earned
    ...
    public function add($id)
    {
        try {
            ...
            // Use individual posted payroll periods (per cutoff), including cutoff in the display name
                ...
                ->select(
                    ...
                    DB::raw("
                        CONCAT(
                            b.name,
                            ' (',
                            DATENAME(MONTH, a.release_date),
                            ' ',
                            DATEPART(YEAR, a.release_date),
                            ' - ',
                            c.name,
                            ') '
                        ) as name
            ...
            // Attendance window used to scope OT records shown/added for this payroll period
            // (Only apply when present; for id=0 dummy payload these are empty strings.)
            $attendance_start_date = $data[0]->attendance_start_date ?? null;
            $attendance_end_date = $data[0]->attendance_end_date ?? null;
            ...
            $employees = DB::table('overtime_payroll_headers as a')
                    ...
                    DB::raw("
                        ISNULL(
                            (
                                SELECT SUM(ISNULL(ts.Adjustment_Amount_OT_Holiday, 0))
                                FROM time_data_summary ts
                                WHERE ts.employee_id = b.employee_id
                                  AND ts.payroll_period_id = a.payroll_period_id
                            ),
                            0
                        ) as ot_adjustment
                    "),
                ...
                ->when($attendance_start_date && $attendance_end_date, function ($q) use ($attendance_start_date, $attendance_end_date) {
                    $q->whereBetween('e.date', [$attendance_start_date, $attendance_end_date]);
                })
                ->groupBy([
                    'a.id',
                    'a.payroll_period_id',
            ...
            PayrollBenefitsEmployeeScope::apply($employee_unselected, 'b');
            $employee_unselected = $employee_unselected
                // Only show employees with OT records inside selected payroll period window
                ->when($attendance_start_date && $attendance_end_date, function ($q) use ($attendance_start_date, $attendance_end_date) {
                    $q->whereBetween('f.date', [$attendance_start_date, $attendance_end_date]);
                })
                // Exclude employees already added to this overtime payroll (by employee + overtime type)
                // Note: overtime_payroll_details.overtime_id is currently 0 for inserted rows, so we can't exclude by overtime_id.
                ->whereNotExists(function ($q) use ($id) {
                    $q->select(DB::raw(1))
                        ->from('overtime_payroll_details as opd')
                        ->whereColumn('opd.employee_id', 'b.id')
                        ->whereColumn('opd.overtime_type_id', 'f.overtime_type_id')
                        ->where('opd.overtime_payroll_id', $id);
                })
                ->groupBy(
                    'b.id',
                    'b.is_encrypted',
            ...
            $overtime_types = DB::table('overtime_applications as f')
                ...
                // Scope totals to selected payroll period window
                ->when($attendance_start_date && $attendance_end_date, function ($q) use ($attendance_start_date, $attendance_end_date) {
                    $q->whereBetween('f.date', [$attendance_start_date, $attendance_end_date]);
                })
                ->groupBy(
    ...
    public function deleteOvertimePayroll($id)
    {
        try {
            // Delete details first to maintain referential integrity
            DB::table('overtime_payroll_details')
                ->where('overtime_payroll_id', $id)
                ->delete();

            // Delete header
            DB::table('overtime_payroll_headers')
                ->where('id', $id)
                ->delete();
            ...
            $data_audit = array(
                ...
                'menu' => 'Overtime Payroll',
                'activity' => 'Delete',
                'description' => 'Deleted Overtime Payroll.',
            );

            Audit::create($data_audit);

            return $this->successResponse(['id' => $id], 'Successfully deleted overtime payroll!');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to delete overtime payroll: ' . $e->getMessage());
        }
    }
