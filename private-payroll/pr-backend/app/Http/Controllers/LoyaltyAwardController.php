...
class LoyaltyAwardController extends Controller
{
        ...
        if ($id == 0) {
            $request->validate(
                ...
                [
                    ...
                    'month_id.unique' => "Loyalty record already exists for this month and year.",
                ]
            );
        }
    ...
    {
        try {
            // Prevent deleting posted records
            $header = DB::table('loyalty_award_header')->where('id', $id)->first();
            if (!$header) {
                return $this->notFoundResponse('Loyalty award record not found.');
            }

            if ($header->posted) {
                return $this->errorResponse('Cannot delete a posted loyalty award. Please unpost it first.', 400);
            }

            // Delete related details first
            DB::table('loyalty_award_details')->where('loyalty_award_header_id', $id)->delete();

            // Delete header
            DB::table('loyalty_award_header')->where('id', $id)->delete();
            ...
            $data_audit = [
                ...
                'menu'    => 'Loyalty Award',
                'activity' => 'Delete',
                'description' => 'Deleted Loyalty Award record.',
            ];

            Audit::create($data_audit);

            return $this->successResponse(null, 'Loyalty award deleted successfully');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to delete loyalty award: ' . $e->getMessage());
        }
    }
    ...
    public function loyalty_award_report()
    {
        ...
        // Query posted loyalty award headers instead of payroll periods
        $PayrollPeriodType = DB::table('loyalty_award_header as a')
            ...
            ->where('a.posted', true)  // Only show posted loyalty awards
            ->orderBy('a.year', 'desc')
            ->orderBy('a.month_id', 'desc')
            ->get();
    ...
    public function print(Request $request)
    {
        ...
        $filename = 'loyalty_award_report_' . ($request->payroll_period_id ?? 'period') . '_' . date('Y-m-d') . '.pdf';
    ...
    public function loyalty_award_employee(Request $request, $month_id, $branch_id, $year)
    {
        $app_key = env("APP_KEY", "");
        $payrollId = (int) $request->query('payroll_id', 0);
        ...
        PayrollBenefitsEmployeeScope::apply($employees, 'employees');
        $employees = $employees
            ->whereMonth('date_hired', $month_id)
            ->when($payrollId > 0, function ($query) use ($payrollId) {
                $query->whereNotIn('employees.id', function ($subQuery) use ($payrollId) {
                    $subQuery->select('employee_id')
                        ->from('loyalty_award_details')
                        ->where('loyalty_award_header_id', $payrollId);
                });
            })
            ->orderBy('employees.first_name', 'asc')
            ->get();
