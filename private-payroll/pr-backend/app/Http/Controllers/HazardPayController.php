...
class HazardPayController extends Controller
{
    ...
    public function loadHazard()
    {
        try {
            $data = DB::table('hazard_pay_headers as a')
                ->leftJoin('divisions as b', 'a.department_id', '=', 'b.id')
    ...
    public function addHazard(Request $request, $id)
    {
        try {
            $app_key = env("APP_KEY", "");
            ...
            if ($id != 0) {
                if (!$divisionId) {
                    $divisionId = DB::table('hazard_pay_headers')->where('id', $id)->value('department_id');
                }
                $data = DB::table('hazard_pay_headers as a')
                ...
                $hazardEmployees = $this->hazardAvailableEmployees($request, (int) $id, $divisionId);
                ...
                return $this->successResponse([
                    ...
                    'months' => $months,
                    'data' => $data,
                    'hazard_employees' => $hazardEmployees,
                    'available_employees' => $hazardEmployees,
                ], 'Hazard pay data retrieved successfully');
            } else {
                $hazardEmployees = $this->hazardAvailableEmployees($request, 0, $divisionId);
    ...
    public function storeHazard(Request $request, $id)
    {
        try {
            ...
            $data_audit = array(
                ...
                'menu'    => 'Hazard Pay',
                'activity' => 'Create',
                'description' => 'Created Hazard Pay informations.',
            );
    ...
    public function storeHazardEmployees(Request $request, $id)
    {
        ...
        try {
            ...
            $selectedSetups = collect($employee_data['select'] ?? [])->filter()->unique()->all();

            // Replace existing employees with the payload coming from the frontend to avoid duplicates
            DB::table('hazard_pay_details')
                ->where('hazard_pay_id', $id)
                ->delete();

            if (isset($employee_data['hazard_pay_setup_id']) && is_array($employee_data['hazard_pay_setup_id'])) {
                $arr_len = count($employee_data['hazard_pay_setup_id']);
                for ($i = 0; $i < $arr_len; $i++) {
                    $hazard_pay_setup_id = $employee_data['hazard_pay_setup_id'][$i] ?? null;
                    if ($hazard_pay_setup_id !== null && in_array($hazard_pay_setup_id, $selectedSetups)) {
                        $data = [
                            'hazard_pay_setup_id' => $hazard_pay_setup_id,
                            'hazard_pay_id' => $id,
                            'employee_id' => $employee_data['id'][$i] ?? null,
                            'no_of_days' => $employee_data['no_of_days'][$i] ?? 0,
                            'salary_grade_id' => $employee_data['salary_grade_id'][$i] ?? null,
                            'salary_step_id' => $employee_data['salary_step_id'][$i] ?? null
                        ];
    ...
    public function deleteHazardPayHeader($id)
    {
        try {
            // First delete all related hazard pay details (employees)
            DB::table('hazard_pay_details')->where('hazard_pay_id', $id)->delete();
            
            // Then delete the hazard pay header
            DB::table('hazard_pay_headers')->where('id', $id)->delete();
            ...
            $data_audit = array(
                ...
                'menu'    => 'Hazard Pay',
                'activity' => 'Delete',
                'description' => 'Deleted Hazard Pay Header and all related employees.',
            );

            Audit::create($data_audit);

            return $this->successResponse(null, 'Successfully deleted hazard pay record!');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to delete hazard pay record: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            $divisions = ReportDivisionFilter::divisionsWithHazardPayData();
            $months = DB::table('months')->orderBy('id', 'asc')->get();
            $records = DB::table('hazard_pay_headers as a')
                ->join('hazard_pay_details as d', 'a.id', '=', 'd.hazard_pay_id')
                ...
                ->select(
                    ...
                    'a.month_id',
                    'a.year',
                    DB::raw('COALESCE(c.name, dept.name) as division'),
    ...
    public function print(Request $request)
    {
        try {
            ...
            $hazard = DB::table('hazard_pay_headers as a')
                ...
                ->join('divisions as i', 'a.department_id', '=', 'i.id')
            ...
            $filename = 'hazard_pay_report_' . $divisionId . '_' . $request->month_id . '_' . $request->year . '_' . date('Y-m-d') . '.pdf';
    ...
    /**
     * Active employees available to add to a hazard pay header (filtered by division).
     */
    private function hazardAvailableEmployees(Request $request, int $hazardPayId = 0, $divisionId = null)
    {
        $app_key = env('APP_KEY', '');
        $divisionId = $divisionId ?? ReportDivisionFilter::resolveId($request);
        ...
        $query->when($divisionId, function ($q) use ($divisionId) {
                return $q->where('employees.division_id', $divisionId);
            });

        if ($hazardPayId > 0) {
            $query->whereNotIn('employees.id', function ($sub) use ($hazardPayId) {
                $sub->select('employee_id')
                    ->from('hazard_pay_details')
                    ->where('hazard_pay_id', $hazardPayId);
            });
        }

        return $query->orderBy('employees.first_name', 'asc')->get();
    }
}
