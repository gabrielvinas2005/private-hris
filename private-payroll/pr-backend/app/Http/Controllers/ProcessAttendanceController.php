...
class ProcessAttendanceController extends Controller
{
    ...
    private function payrollPeriodEmploymentTypeIds(int $payrollPeriodId): array
    {
    ...
    /** Active employees for attendance — same roster as Process Payroll (payroll_period_Etype). */
    private function employeesForPayrollPeriod(int $payrollPeriodId)
    {
        $employmentTypeIds = $this->payrollPeriodEmploymentTypeIds($payrollPeriodId);
    ...
    public function process(Request $request)
    {
        ...
        $payrollPeriodId = (int) $request->payroll_period_id;
        $employmentTypeIds = $this->payrollPeriodEmploymentTypeIds($payrollPeriodId);
        if (empty($employmentTypeIds)) {
            return $this->errorResponse(
                'No employment types are assigned to this payroll period. Configure payroll_period_Etype first.',
                400
            );
        }

        // Same employee pool as Process Payroll: payroll_period_Etype for this period.
        $employees = $this->employeesForPayrollPeriod($payrollPeriodId);
