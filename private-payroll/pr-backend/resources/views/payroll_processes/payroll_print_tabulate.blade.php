...
<body>
    <div>
        ...
        <div>
            <p style="text-align: left;margin-top: 50px;margin-left: 42px;font-size: 12px;">We acknowledge receipt
                of
                cash shown
        ...
        <div>
            <table style="margin: 5px 0px 10px 0px;width: 100%;">
                <thead>
                    <tr>
                        ...
                        <th rowspan="2" style="width: 150px;">DESIGNATION</th>
                        ...
                        @foreach ($income_headers as $inc_hdr)
                            <th rowspan="2" style="width: 40px;">{{ strtoupper($inc_hdr->income) }}</th>
                        @endforeach
                        <th rowspan="2" style="width: 40px;">GROSS AMOUNT</th>
                        <th colspan="{{ 4 + count($deduction_headers) }}">DEDUCTIONS</th>
                        <th rowspan="2" style="width: 80px;">OTHER DEDUCTIONS</th>
                        <th colspan="2">NET AMOUNT RECEIVED</th>
                        <th rowspan="2" style="width: 20px;">NO.</th>
                    </tr>
                    <tr>
                        <th style="width: 30px;">GSIS PREMIUM</th>
                        <th style="width: 30px;">HDMF PREMIUM</th>
                        <th style="width: 40px;">TAX WITHHELD</th>
                        <th style="width: 30px;">PHILHEALTH</th>
                        @foreach ($deduction_headers as $deduc_hdr)
                            <th style="width: 40px;">{{ strtoupper($deduc_hdr->deduction) }}</th>
                        @endforeach
                        <th style="width: 40px;"><b>15TH</b></th>
                        <th style="width: 40px;"><b>{{ date('d', strtotime($data[0]->attendance_end_date)) }}TH</b>
                        </th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    @foreach ($payrolls as $pay)
                        @if ($pay->department_id != $dep_id)
                            @php($ctr = 1)
                            @php($ctr_1 = 1)
                            <tr style="background-color: lightgray;">
                                <td colspan="{{ 14 + count($income_headers) + count($deduction_headers) }}"
                                    style="text-align: left;color: black;font-weight: bold;">
                                    {{ strtoupper($pay->department) }}
                                </td>
                            </tr>
                            @php($dep_id = $pay->department_id)
                        @endif
                        <tr>
                            <td>{{ $ctr++ }}</td>
                            ...
                            @foreach ($income_headers as $inc_hdr)
                                ...
                                @foreach ($incomes as $inc)
                                    @if ($inc->employee_id == $pay->employee_id && $inc->income_id == $inc_hdr->income_id && $inc->amount > 0)
                                        <td class="right-align">{{ number_format($inc->amount, 2, '.', ',') }}</td>
                                        @php($x = true)
                                    @break

                                @elseif($inc->employee_id == $pay->employee_id && $inc->income_id == $inc_hdr->income_id && $inc->amount == 0)
                                    <td class="right-align">{{ number_format($inc->amount, 2, '.', ',') }}</td>
                                    @php($x = true)
                                @break
                            @endif
                    ...
                    @foreach ($deduction_headers as $deduc_hdr)
                        ...
                        @foreach ($deductions as $deduc)
                            @if ($deduc->employee_id == $pay->employee_id && $deduc->deduction_id == $deduc_hdr->deduction_id)
                                <td class="left-align">{{ number_format($deduc->amount, 2, '.', ',') }}</td>
                                @php($y = true)
                            @break

                        @elseif($deduc->employee_id == $pay->employee_id && $deduc->deduction_id == $deduc_hdr->deduction_id)
                            <td class="left-align">{{ number_format($deduc->amount, 2, '.', ',') }}</td>
                            @php($y = true)
                        @break
                    @endif
            ...
            <td>
                <div>
                    <p style="width: 43%;text-align:left;display: inline-block;">Late
                    </p>
                    <p style="width: 43%;text-align:right;display: inline-block;">
                        {{ number_format($pay->late_amount, 2, '.', ',') }}</p>
                </div>
                <div>
                    <p style="width: 43%;text-align:left;display: inline-block;">Undertime</p>
                    <p style="width: 43%;text-align:right;display: inline-block;">
                        {{ number_format($pay->ut_amount, 2, '.', ',') }}
                    </p>
                </div>
                <div>
                    <p style="width: 43%;text-align:left;display: inline-block;">Absent</p>
                    <p style="width: 43%;text-align:right;display: inline-block;">
                        {{ number_format($pay->absent_amount - $pay->lwop_amount, 2, '.', ',') }}
                    </p>
                </div>
                <div>
                    <p style="width: 43%;text-align:left;display: inline-block;">LWOP</p>
                    <p style="width: 43%;text-align:right;display: inline-block;">
                        {{ number_format($pay->lwop_amount, 2, '.', ',') }}
                    </p>
                </div>
                @foreach ($deductions_others as $deduc_oth)
                    @if ($deduc_oth->employee_id == $pay->employee_id)
                        <div>
                            <p style="width: 43%;text-align:left;display: inline-block;">
                                {{ $deduc_oth->name }}
                            </p>
                            <p style="width: 43%;text-align:right;display: inline-block;">
                                {{ number_format($deduc_oth->amount, 2, '.', ',') }}
                            </p>
                        </div>
                    @endif
                @endforeach
                @foreach ($pending_deductions as $pending_deduction)
                    @if ($pending_deduction->employee_id == $pay->employee_id)
                        <div>
                            <p style="width: 43%;text-align:left;display: inline-block;">
                                {{ $pending_deduction->deduction_type }}
                            </p>
                            <p style="width: 43%;text-align:right;display: inline-block;">
                                {{ number_format($pending_deduction->amount_deducted, 2, '.', ',') }}
                            </p>
                        </div>
                    @endif
                @endforeach
            ...
            <td class="right-align">{{ number_format(round($pay->net_pay / 2, 0), 2, '.', ',') }}</td>
            <td>{{ $ctr_1++ }}</td>
        </tr>
...
<p style="float: right;font-size: 8px;margin-bottom: 0px;margin-right: 20px;">{{ $footer['document_no'] }}
</p>
<br>
<p style="float: right;font-size: 8px;margin-top: 0px;margin-right: 20px;">{{ $footer['revision'] }}</p>
</div>
</div>
</body>
