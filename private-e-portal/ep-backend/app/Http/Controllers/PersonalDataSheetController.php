...
class PersonalDataSheetController extends Controller
{
    ...
    private function formatEducationData($educationData)
    {
        foreach ($educationData as $education) {
            ...
            // Some databases use `program` instead of `degree`. Fallback accordingly.
            $education->degree = $this->formatField(($education->degree ?? null) !== null ? $education->degree : ($education->program ?? ''));
    ...
    public function print(Request $request)
    {
        try {
            ...
            $pdf = PDF::loadView('pds.pds_print', compact(
                'info',
                'children',
                'educations_elem',
                'educations_sec',
