...
class EmployeeRequestController extends Controller
{
    ...
    {
        try {
            // Check if current date is within update_201_schedule date range
    ...
    public function autosave(Request $request, $employee_id)
    {
        try {
            // Check if current date is within update_201_schedule date range
            $today = now()->format('Y-m-d');
            ...
            if ($employeeRequest) {
                ...
                // Create new request
                $request_id = DB::table('employee_requests')->max('id');
                $request_id = ($request_id ? $request_id : 0) + 1;
            ...
            // Prepare employee info from request
            $employee_info = [
                'employee_id' => $employee_id,
                'employee_no' => $request->employee_no ?? '',
                'access_no' => $request->access_no ?? '',
                ...
                'plantilla_id' => $request->plantilla_id ?? 0,
                'date_hired' => $request->date_hired ?? '',
                // Payroll
            ...
            // Update or insert into employees_temps
            DB::table('employees_temps')->updateOrInsert(
                ['request_id' => $request_id],
                $employee_info
            );
            ...
            if ($request->has('children') && is_array($request->children)) {
                ...
                foreach ($request->children as $child) {
                    DB::table('employee_children_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'child_name' => $child['child_name'] ?? '',
                        'child_middlename' => $child['child_middlename'] ?? '',
            ...
            if ($request->has('educations') && is_array($request->educations)) {
                ...
                foreach ($request->educations as $education) {
                    // Convert date strings to years if needed (from/to are integers in DB)
                    $fromYear = $education['from'] ?? null;
                    $toYear = $education['to'] ?? null;
                    
                    // If it's a date string, extract the year
                    if ($fromYear && is_string($fromYear) && preg_match('/^\d{4}-\d{2}-\d{2}/', $fromYear)) {
                        $fromYear = (int) substr($fromYear, 0, 4);
                    } elseif ($fromYear && !is_numeric($fromYear)) {
                        $fromYear = null;
                    } else {
                        $fromYear = $fromYear ? (int) $fromYear : null;
                    }
                    
                    if ($toYear && is_string($toYear) && preg_match('/^\d{4}-\d{2}-\d{2}/', $toYear)) {
                        $toYear = (int) substr($toYear, 0, 4);
                    } elseif ($toYear && !is_numeric($toYear)) {
                        $toYear = null;
                    } else {
                        $toYear = $toYear ? (int) $toYear : null;
                    }
                    
                    DB::table('employee_educations_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'academic_level_id' => $education['academic_level_id'] ?? 0,
                        'school_name' => $education['school_name'] ?? '',
                        'program' => $education['program'] ?? '',
                        'from' => $fromYear,
                        'to' => $toYear,
                        'graduated_year' => $education['graduated_year'] ?? null,
                        'units_earned' => $education['units_earned'] ?? 0,
            ...
            // Save Service Records
            if ($request->has('serviceRecords') && is_array($request->serviceRecords)) {
                DB::table('service_records_temps')->where('request_id', $request_id)->delete();
                foreach ($request->serviceRecords as $record) {
                    DB::table('service_records_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
            ...
            if ($request->has('workExperiences') && is_array($request->workExperiences)) {
                ...
                foreach ($request->workExperiences as $experience) {
                    DB::table('employee_employment_records_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'work_start_date' => $experience['work_start_date'] ?? null,
                        'work_end_date' => $experience['work_end_date'] ?? null,
                        'work_company' => $experience['work_company'] ?? '',
                        'monthly_salary' => $experience['monthly_salary'] ?? 0,
                        'salary_grade_step' => $experience['salary_grade_step'] ?? '',
                        'status_of_appointment' => $experience['status_of_appointment'] ?? '',
                        'position' => $experience['position'] ?? '',
                        'government_service_id' => $experience['government_service_id'] ?? 0,
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
            ...
            // Save Eligibilities (Examinations)
            if ($request->has('eligibilities') && is_array($request->eligibilities)) {
                DB::table('employee_examinations_temps')->where('request_id', $request_id)->delete();
                foreach ($request->eligibilities as $eligibility) {
                    // Try to find eligibility_id from eligibility name if provided
                    $eligibility_id = 0;
                    if (!empty($eligibility['eligibility'])) {
                        $eligibility_record = DB::table('eligibilities')
                            ->where('name', $eligibility['eligibility'])
                            ->first();
                        if ($eligibility_record) {
                            $eligibility_id = $eligibility_record->id;
                        }
                    } else {
                        $eligibility_id = $eligibility['eligibility_id'] ?? 0;
                    }

                    DB::table('employee_examinations_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'eligibility_id' => $eligibility_id,
                        'exam_rating' => $eligibility['exam_rating'] ?? 0,
                        'exam_date' => $eligibility['exam_date'] ?? null,
                        'place_of_exam' => $eligibility['place_of_exam'] ?? '',
                        'license_number' => $eligibility['license_number'] ?? '',
                        'date_released' => $eligibility['date_released'] ?? null,
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
            ...
            if ($request->has('trainings') && is_array($request->trainings)) {
                ...
                foreach ($request->trainings as $training) {
                    DB::table('employee_trainings_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'training' => $training['training'] ?? '',
                        'training_from' => $training['training_from'] ?? null,
                        'training_to' => $training['training_to'] ?? null,
                        'hours' => $training['hours'] ?? 0,
                        'sponsored_by' => $training['sponsored_by'] ?? '',
                        'learning_id' => $training['learning_id'] ?? ($training['learning'] ?? 0), // Map learning to learning_id
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
            ...
            // Save Voluntary Works (Organizations)
            if ($request->has('voluntaryWorks') && is_array($request->voluntaryWorks)) {
                DB::table('employee_organizations_temps')->where('request_id', $request_id)->delete();
                foreach ($request->voluntaryWorks as $work) {
                    $orgData = [
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'organization' => $work['organization'] ?? '',
                        'org_from' => $work['org_from'] ?? null,
                        'org_to' => $work['org_to'] ?? null,
                        'org_hours' => $work['org_hours'] ?? 0,
                        'org_position' => $work['org_position'] ?? '',
                        'created_at' => now(),
                        'updated_at' => now()
                    ];
                    // Add organization_address if column exists
                    if (Schema::hasColumn('employee_organizations_temps', 'organization_address')) {
                        $orgData['organization_address'] = $work['organization_address'] ?? '';
                    }
                    DB::table('employee_organizations_temps')->insert($orgData);
                }
            }

            // Save Recognitions
            if ($request->has('recognitions') && is_array($request->recognitions)) {
                DB::table('employee_recognations_temps')->where('request_id', $request_id)->delete();
                foreach ($request->recognitions as $recognition) {
                    DB::table('employee_recognations_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'recognation' => $recognition['recognation'] ?? '',
                        'created_at' => now(),
                        'updated_at' => now()
                    ]);
                }
            ...
            if ($request->has('skills') && is_array($request->skills)) {
                ...
                foreach ($request->skills as $skill) {
                    DB::table('employee_skills_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'skill' => $skill['skill'] ?? '',
                        'created_at' => now(),
            ...
            // Save Memberships
            if ($request->has('memberships') && is_array($request->memberships)) {
                DB::table('employee_memberships_temps')->where('request_id', $request_id)->delete();
                foreach ($request->memberships as $membership) {
                    DB::table('employee_memberships_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
            ...
            if ($request->has('references') && is_array($request->references)) {
                ...
                foreach ($request->references as $reference) {
                    DB::table('employee_references_temps')->insert([
                        'request_id' => $request_id,
                        'employee_id' => $employee_id,
                        'ref_name' => $reference['ref_name'] ?? '',
                        'ref_address' => $reference['ref_address'] ?? '',
                        'ref_occupation' => $reference['ref_occupation'] ?? '',
                        'ref_contact_no' => $reference['ref_contact_no'] ?? '',
            ...
            return $this->serverErrorResponse('Failed to autosave: ' . $e->getMessage());
        }
    }
