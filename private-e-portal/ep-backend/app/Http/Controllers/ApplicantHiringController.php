...
class ApplicantHiringController extends Controller
{
    ...
    public function download($id)
    {
        try {
            ...
            $applicantNo = $applicants[0]->applicant_no;
            $attachmentName = $applicants[0]->attachment_name;

            // Preferred: check the actual stored file on the local disk (storage/app)
            $relativePath = 'resume/' . $applicantNo . '_' . $attachmentName;
            $pathToFile = storage_path('app/' . $relativePath);

            // Fallback 1: use the saved absolute path from DB (older records)
            if (!file_exists($pathToFile) && !empty($applicants[0]->path) && file_exists($applicants[0]->path)) {
                $pathToFile = $applicants[0]->path;
            }

            // Fallback 2: Storage existence check (handles different path separators)
            if (!file_exists($pathToFile) && Storage::disk('local')->exists($relativePath)) {
                $pathToFile = storage_path('app/' . $relativePath);
            }
            ...
            // Return the file directly for preview/download
