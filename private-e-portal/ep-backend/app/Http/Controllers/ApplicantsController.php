...
class ApplicantsController extends Controller
{
    ...
    {
        try {
            // Get applicants from applicant_headers and applicant_details tables
            // Shortlisted applicants have application_status_id = 5 (For Hiring) or are in applicant_shortlisted table
            $data = DB::table('applicant_details as ad')
                ->join('applicant_headers as ah', 'ad.applicant_id', '=', 'ah.id')
                ->join('application_status', 'application_status.id', '=', 'ad.application_status_id')
                ->leftJoin('applicant_shortlisted as asl', 'asl.applicant_id', '=', 'ah.id')
                ->select(
                    'ah.id',
                    DB::raw("CONCAT(ah.first_name, ' ', ah.last_name) as name"),
                    'ah.application_date as date_applied',
                    'ad.position_applied_id',
                    'application_status.name as status',
                    'ad.application_status_id',
                    DB::raw("CASE WHEN asl.id IS NOT NULL THEN 1 ELSE 0 END as is_shortlisted"),
                    'asl.rating'
                )
                ->orderBy('asl.rating', 'desc')
                ->orderBy('ah.first_name', 'asc')
                ->where('ad.position_applied_id', $plantilla_id)
                ->where('ad.is_plantilla', 1)
                ->where(function ($query) {
                    $query->where('ad.application_status_id', 5) // Status 5 = For Hiring (shortlisted)
                        ->orWhereNotNull('asl.id'); // Or in applicant_shortlisted table
                })
                ->paginate(20);

            return $this->successResponse($data, 'Shortlisted applicants retrieved successfully');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to retrieve shortlisted applicants: ' . $e->getMessage());
        }
    }
    ...
    {
        try {
            // Get applicant info from applicant_headers and applicant_details
            $applicant = DB::table('applicant_headers as ah')
                ->join('applicant_details as ad', 'ad.applicant_id', '=', 'ah.id')
                ->join('application_status', 'application_status.id', '=', 'ad.application_status_id')
                ->select(
                    'ah.id',
                    DB::raw("CONCAT(ah.first_name, ' ', ah.last_name) as name"),
            ...
            $attachments = DB::table('applicant_attachments')
                ->select('id', 'attachment_name', 'path', 'created_at')
                ->where('applicant_id', $id)
                ->orderBy('created_at', 'desc')
                ->get();

            // Examination schedules & results (per applicant)
            $examination_schedules = DB::table('applicant_examination_headers as c')
                ->join('examination_schedule_header as a', 'c.exam_schedule_id', '=', 'a.id')
                ->join('examination_setup_header as b', 'a.exam_id', '=', 'b.id')
                ->leftJoin('exam_categories as d', 'b.category_id', '=', 'd.id')
                ->select(
                    'a.id',
                ...
                ->orderBy('a.exam_date_from', 'desc')
                ->get();

            // Interview schedules & panel ratings (per applicant)
            $interview_schedules = DB::table('applicant_interview_headers as a')
                ->join('interview_applicants as b', 'a.id', '=', 'b.interview_id')
                ->leftJoin('interview_levels as c', 'a.panel_group_level', '=', 'c.id')
                ->select(
                    'a.id',
                    'a.panel_group',
                    ...
                    'a.end_time',
                    'a.posted',
                    'b.is_complete_interview',
                    'b.is_cancelled_interview',
                    'b.is_expired_interview',
                    DB::raw("case
            ...
            $interview_panels = [];
            $interview_panel_attachments = [];
            $interview_ids = $interview_schedules->pluck('id')->toArray();
            if (!empty($interview_ids)) {
                $app_key_for_panels = env("APP_KEY", "");

                $interview_panels = DB::table('interview_panels as p')
                    ->join('employees as e', 'p.employee_id', '=', 'e.id')
                    ->leftJoin('interview_panel_ratings as r', function ($join) use ($id) {
                        $join->on('r.interview_id', '=', 'p.interview_id')
                    ...
                    ->select(
                        ...
                        'p.interview_id',
                        'p.employee_id',
                        'p.is_complete_rating',
                        DB::raw("CASE WHEN ISNULL(e.is_encrypted,0) = 0 THEN
                            CONCAT(e.first_name, ' ', e.last_name)
                        ELSE
                            CONCAT(RTRIM([dbo].[ufn_DecryptString](e.first_name,'" . $app_key_for_panels . "')), ' ', RTRIM([dbo].[ufn_DecryptString](e.last_name,'" . $app_key_for_panels . "')))
                        END as panel_name"),
                        DB::raw("ISNULL(r.bei_rating,0) as bei_rating"),
                        DB::raw("ISNULL(r.competency_rating,0) as competency_rating"),
                        DB::raw("CAST((ISNULL(r.bei_rating,0) + ISNULL(r.competency_rating,0)) / 2.0 AS decimal(18,2)) as average_rating")
                    )
                    ->whereIn('p.interview_id', $interview_ids)
                    ->orderBy('p.interview_id', 'asc')
                    ->orderBy('panel_name', 'asc')
                    ->get();

                // Attachments uploaded by panels for this applicant across these interviews
                $interview_panel_attachments = DB::table('interview_panel_attachments as a')
                    ...
                    ->whereIn('a.interview_id', $interview_ids)
                    ->orderBy('a.interview_id', 'asc')
                    ->orderBy('panel_name', 'asc')
                    ->orderBy('a.created_at', 'asc')
                    ->get();
            }

            // Get PDS data - check if applicant has an employee record
            $employee = DB::table('employees')
                ->where('employee_no', $applicant->applicant_no)
                ->first();

            // Initialize PDS data structure
            $pds_data = [
                'personal_info' => null,
                'family' => [],
                'education' => [],
            ...
            if ($employee) {
                ...
                // Get personal information from employee table with all family fields
                $personal_info = DB::table('employees')
                    ->leftJoin('name_prefixes as np', 'employees.name_prefix_id', '=', 'np.id')
                    ->leftJoin('name_suffixes as ns', 'employees.name_suffix_id', '=', 'ns.id')
                    ->leftJoin('genders as g', 'employees.gender_id', '=', 'g.id')
                    ->leftJoin('civil_status as cs', 'employees.civil_status_id', '=', 'cs.id')
                    ->leftJoin('citizenships as c', 'employees.citizenship_id', '=', 'c.id')
                    ->leftJoin('religions as r', 'employees.religion_id', '=', 'r.id')
                    ->leftJoin('blood_types as bt', 'employees.blood_type_id', '=', 'bt.id')
                    ->leftJoin('name_prefixes as fp', 'employees.father_name_prefix_id', '=', 'fp.id')
                    ->leftJoin('name_suffixes as fs', 'employees.father_name_suffix_id', '=', 'fs.id')
                    ->leftJoin('name_prefixes as mp', 'employees.mother_name_prefix_id', '=', 'mp.id')
                    ->leftJoin('name_suffixes as ms', 'employees.mother_name_suffix_id', '=', 'ms.id')
                    ->leftJoin('name_prefixes as sp', 'employees.spouse_name_prefix_id', '=', 'sp.id')
                    ->leftJoin('name_suffixes as ss', 'employees.spouse_name_suffix_id', '=', 'ss.id')
                    ->select(
                        'employees.id',
                        'employees.employee_no',
                        'employees.email',
                        'employees.mobile_no',
                        'employees.telephone_no',
                        'employees.tin_no',
                        'employees.gsis_no',
                        'employees.sss_no',
                        ...
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.first_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.first_name,'" . $app_key . "')) END as first_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.middle_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.middle_name,'" . $app_key . "')) END as middle_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.last_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.last_name,'" . $app_key . "')) END as last_name"),
                        'ns.name as name_suffix',
                        ...
                        'employees.weight',
                        'employees.ra_region',
                        'employees.ra_province',
                        'employees.ra_city',
                        'employees.ra_barangay',
                        'employees.ra_house_no',
                        'employees.ra_street',
                        'employees.ra_village',
                        'employees.pa_region',
                        'employees.pa_province',
                        'employees.pa_city',
                        'employees.pa_barangay',
                        'employees.pa_house_no',
                        'employees.pa_street',
                        'employees.pa_village',
                        'employees.is_dual_citizent',
                        'employees.by_birth',
                        'employees.by_naturalization',
                        'employees.indicate_country',
                        'employees.father_name_prefix_id',
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.father_first_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.father_first_name,'" . $app_key . "')) END as father_first_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.father_middle_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.father_middle_name,'" . $app_key . "')) END as father_middle_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.father_last_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.father_last_name,'" . $app_key . "')) END as father_last_name"),
                        'employees.father_name_suffix_id',
                        'fp.name as father_name_prefix',
                        'fs.name as father_name_suffix',
                        'employees.mother_name_prefix_id',
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.mother_first_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.mother_first_name,'" . $app_key . "')) END as mother_first_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.mother_middle_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.mother_middle_name,'" . $app_key . "')) END as mother_middle_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.mother_last_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.mother_last_name,'" . $app_key . "')) END as mother_last_name"),
                        'employees.mother_name_suffix_id',
                        'mp.name as mother_name_prefix',
                        'ms.name as mother_name_suffix',
                        'employees.spouse_name_prefix_id',
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.spouse_first_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.spouse_first_name,'" . $app_key . "')) END as spouse_first_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.spouse_middle_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.spouse_middle_name,'" . $app_key . "')) END as spouse_middle_name"),
                        DB::raw("CASE WHEN ISNULL(employees.is_encrypted,0) = 0 THEN employees.spouse_last_name ELSE RTRIM([dbo].[ufn_DecryptString](employees.spouse_last_name,'" . $app_key . "')) END as spouse_last_name"),
                        'employees.spouse_name_suffix_id',
                        'sp.name as spouse_name_prefix',
                        'ss.name as spouse_name_suffix',
                        'employees.spouse_occupation',
                        'employees.spouse_employer',
                        'employees.spouse_business_address'
                    )
                    ->where('employees.id', $employee->id)
                    ->first();

                $pds_data['personal_info'] = $personal_info;

                // Get family information
                $pds_data['family'] = [
                    'father' => [
                        'name_prefix' => $personal_info->father_name_prefix ?? '',
                        'first_name' => $personal_info->father_first_name ?? '',
                        'middle_name' => $personal_info->father_middle_name ?? '',
                        'last_name' => $personal_info->father_last_name ?? '',
                        'name_suffix' => $personal_info->father_name_suffix ?? ''
                    ],
                    'mother' => [
                        'name_prefix' => $personal_info->mother_name_prefix ?? '',
                        'first_name' => $personal_info->mother_first_name ?? '',
                        'middle_name' => $personal_info->mother_middle_name ?? '',
                        'last_name' => $personal_info->mother_last_name ?? '',
                        'name_suffix' => $personal_info->mother_name_suffix ?? ''
                    ],
                    'spouse' => [
                        'name_prefix' => $personal_info->spouse_name_prefix ?? '',
                        'first_name' => $personal_info->spouse_first_name ?? '',
                        'middle_name' => $personal_info->spouse_middle_name ?? '',
                        'last_name' => $personal_info->spouse_last_name ?? '',
                        'name_suffix' => $personal_info->spouse_name_suffix ?? '',
                        'occupation' => $personal_info->spouse_occupation ?? '',
                        'employer' => $personal_info->spouse_employer ?? '',
                        'business_address' => $personal_info->spouse_business_address ?? ''
                    ],
                ...
                // Get education
                $pds_data['education'] = DB::table('employee_educations as e')
                    ->select('e.*')
                    ->where('e.employee_id', $employee->id)
                    ->orderBy('e.graduated_year', 'desc')
                    ->get();

                // Map academic_level_id to readable names
                $academicLevels = [
                    0 => 'Elementary',
                    1 => 'Secondary',
                    2 => 'Vocational',
                ...
                // Add academic_level name to each education record
                foreach ($pds_data['education'] as $edu) {
                    $edu->academic_level = $academicLevels[$edu->academic_level_id] ?? 'Unknown';
                }

                // Get employment records from Work_Experience table
                // Work_Experience is linked by Reference_id which matches applicant_no
                $pds_data['employment'] = DB::table('Work_Experience')
                    ->where('Reference_id', $applicant->applicant_no)
                    ->get();

                // Get examinations/eligibilities
                $pds_data['examinations'] = DB::table('employee_examinations as ex')
                    ->leftJoin('eligibilities as el', 'ex.eligibility_id', '=', 'el.id')
                    ->select(
                        'ex.*',
                        'el.name as eligibility'
                    )
                    ->where('ex.employee_id', $employee->id)
                    ->get();

                // Get trainings
                $pds_data['trainings'] = DB::table('employee_trainings as t')
                    ->leftJoin('learnings as l', 't.learning_id', '=', 'l.id')
                    ->select(
                        't.*',
                        'l.name as learning_type'
                    )
                    ->where('t.employee_id', $employee->id)
                    ->get();

                // Get organizations
                $pds_data['organizations'] = DB::table('employee_organizations')
                    ->where('employee_id', $employee->id)
                    ->get();

                // Get recognitions
                $pds_data['recognitions'] = DB::table('employee_recognations')
                    ->where('employee_id', $employee->id)
                    ->get();

                // Get skills
                $pds_data['skills'] = DB::table('employee_skills')
                    ->where('employee_id', $employee->id)
                    ->get();

                // Get memberships
                $pds_data['memberships'] = DB::table('employee_memberships')
                    ->where('employee_id', $employee->id)
                    ->get();

                // Get references
                $pds_data['references'] = DB::table('employee_references')
                    ->where('employee_id', $employee->id)
                    ->get();
            }
            ...
            return $this->successResponse([
                ...
                'application_status_id' => $applicant->application_status_id,
                ...
                'interview_panel_attachments' => $interview_panel_attachments
            ], 'Applicant info retrieved successfully');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to retrieve applicant info: ' . $e->getMessage());
        }
    }
