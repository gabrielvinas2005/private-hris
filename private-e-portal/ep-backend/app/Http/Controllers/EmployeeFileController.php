    ...
    public function index($id = null)
    {
        try {
            ...
            if ($emp_id_data->isEmpty()) {
                ...
                // Use the already loaded address reference data
            ...
            // IPCR details (only if table/column exists in current DB)
            ...
            // Reference dropdowns
            $genders = Schema::hasTable('genders')
                ? DB::table('genders')->orderBy('name')->get()
                : collect();

            $blood_types = Schema::hasTable('blood_types')
                ? DB::table('blood_types')->orderBy('name')->get()
                : collect();

            $civil_statuses = Schema::hasTable('civil_status')
                ? DB::table('civil_status')->orderBy('name')->get()
                : collect();

            $religions = Schema::hasTable('religions')
                ? DB::table('religions')->orderBy('name')->get()
                : collect();

            $employment_types = Schema::hasTable('employment_types')
                ? DB::table('employment_types')->where('active', true)->orderBy('id', 'asc')->get()
                : collect();
            ...
            if ($schedule) {
                $can_update = true;
                $days_remaining = $schedule->remaining_days ?? 0;
            }

            // Get last update date from employee_requests (most recent request)
            $last_update_date = 'Original Date Created';
            $lastRequest = DB::table('employee_requests')
                ->where('employee_id', $emp_id)
                ->orderBy('request_date', 'desc')
                ->first();
            
            if ($lastRequest && $lastRequest->request_date) {
                $last_update_date = date('F d, Y', strtotime($lastRequest->request_date));
            }
