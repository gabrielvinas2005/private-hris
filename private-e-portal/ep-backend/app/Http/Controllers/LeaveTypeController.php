        ...
        try {
            ...
            for ($i = 0; $i < $arr_len; $i++) {
                if (isset($data['name'][$i]) && $data['name'][$i] != '' && isset($data['accrual_amount'][$i]) && $data['accrual_amount'][$i] != '') {
                    ...
                    // Emergency Leave flag (UI may send as `is_el` or `emergency_leave`)
                    $emergencyLeaveIds = [];
                    if (isset($data['is_el']) && is_array($data['is_el'])) {
                        $emergencyLeaveIds = $data['is_el'];
                    } elseif (isset($data['emergency_leave']) && is_array($data['emergency_leave'])) {
                        $emergencyLeaveIds = $data['emergency_leave'];
                    }

                    $is_el = in_array($data['id'][$i], $emergencyLeaveIds);
            ...
            $data_audit = array(
                ...
                'activity' => 'Update',
                'description' => 'Updated leave type table informations.',
            );
