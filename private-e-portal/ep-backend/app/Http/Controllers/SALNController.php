...
class SALNController extends Controller
{
    ...
    public function index($id)
    {
        ...
        // Prefer employees.id when passed directly; fall back to mapping from users.id
        $emp_id = (!$emp_id_data->isEmpty() && isset($emp_id_data[0]->id) && (int)$emp_id_data[0]->id > 0)
            ? (int)$emp_id_data[0]->id
            : (int)$id;
        ...
        if ($emp_id === 0) {
            ...
            // Load employee info directly by employees.id
            $info = DB::table('users as a')
                ...
                ->where(['b.is_employee' => true, 'b.active' => true, 'b.id' => $emp_id])
                ->get();
        ...
        $user_id = $this->resolveUserId($emp_id, (int)$id);

        $children = DB::table('employee_children')
            ->where('employee_id', $emp_id)
            ->whereNotNull('child_birthdate')
            ->whereRaw("DATEDIFF(year, child_birthdate, GETDATE()) < 18")
            ->get();
        $realProperties = SalnRealProperty::where('user_id', $user_id)->get();
        $personalProperties = SalnPersonalProperty::where('user_id', $user_id)->get();
        $liabilities = SalnLiability::where('user_id', $user_id)->get();
        $business = SalnBusinessInterest::where('user_id', $user_id)->get();
        $relatives = SalnBusinessRelatives::where('user_id', $user_id)->get();
        ...
        $references = DB::table('employee_references')->where('employee_id', $emp_id)->get();
            // IPCR details (guard when column/table missing)
            $ipcr_details = collect();
            if (\Schema::hasTable('ipcr_details') && \Schema::hasColumn('ipcr_details', 'employee_id')) {
        ...
        $realPropertiesTotal = $realProperties->sum('acquisition_cost');
        $personalPropertiesTotal = $personalProperties->sum('acquisition_cost');
        $liabilitiesTotal = $liabilities->sum('outstanding_balance');
    ...
    public function download($id, Request $request)
    {
        try {
            ...
            // Get compliance and filing parameters from request
            $complianceType = $request->query('complianceType', null);
            $complianceDate = $request->query('complianceDate', null);
            $filing = $request->query('filing', null);

            // Resolve employee ID from user ID if needed
            $emp_id_data = DB::table('users as a')
                ->join('employees as b', 'b.employee_no', '=', 'a.employee_no')
                ->selectRaw('case when b.id = null then 0 else b.id end as id')
                ->where('a.id', $id)
            ...
            $ra_region = collect($region_data)->where("regCode", $info[0]->ra_region ?? null)->all();
            $pa_region = collect($region_data)->where("regCode", $info[0]->pa_region ?? null)->all();

            // set collection for address
            $ra_province = collect($province_data)->where("provCode", $info[0]->ra_province ?? null)->all();
            $pa_province = collect($province_data)->where("provCode", $info[0]->pa_province ?? null)->all();

            $ra_city = collect($city_data)->where("citymunCode", $info[0]->ra_city ?? null)->all();
            $pa_city = collect($city_data)->where("citymunCode", $info[0]->pa_city ?? null)->all();

            $ra_brgy = collect($brgy_data)->where("brgyCode", $info[0]->ra_barangay ?? null)->all();
            $pa_brgy = collect($brgy_data)->where("brgyCode", $info[0]->pa_barangay ?? null)->all();
            ...
            $children = DB::table('employee_children as a')
                ...
                ->where('employee_id', $emp_id)
                ->whereNotNull('a.child_birthdate')
                ->whereRaw("DATEDIFF(year, a.child_birthdate, GETDATE()) < 18")
            ...
            // Calculate asOfDate based on compliance type
            $asOfDate = null;
            if ($complianceType && $complianceDate) {
                if ($complianceType === 'annual') {
                    // For annual filing, use December 31 of the year
                    $asOfDate = $complianceDate . '-12-31';
                } else {
                    // For assumption and exit, use the date as is
                    $asOfDate = $complianceDate;
                }
            }
    ...
    /**
     * Resolve the owning user id either from an employee id or a fallback identifier.
     */
    protected function resolveUserId(?int $employeeId, ?int $fallbackId = null): int
    {
        if (!empty($employeeId)) {
            $userFromEmployee = DB::table('users as a')
                ->join('employees as b', 'b.employee_no', '=', 'a.employee_no')
                ->where('b.id', $employeeId)
                ->value('a.id');

            if (!empty($userFromEmployee)) {
                return (int) $userFromEmployee;
            }
        }

        if (!empty($fallbackId)) {
            $directUser = DB::table('users')
                ->where('id', $fallbackId)
                ->value('id');

            if (!empty($directUser)) {
                return (int) $directUser;
            }
        }
