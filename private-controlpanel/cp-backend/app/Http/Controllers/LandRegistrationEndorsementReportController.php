...
class LandRegistrationEndorsementReportController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            return $this->successResponse($data, 'Land registration endorsements retrieved successfully');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to retrieve land registration endorsements: ' . $e->getMessage());
        }
    }
    ...
    public function print(Request $request)
    {
        try {
            ...
            $pdf = PDF::loadView('land_registration_endorsements.land_registration_endorsement_print', compact('employees', 'incomes', 'salary_word', 'annual_salary', 'signatories', 'image'))->setOptions(['defaultFont' => 'sans-serif']);
            $pdf->setPaper('A4');
            
            $pdf_content = $pdf->output();
            $filename = 'land_registration_endorsement_' . $request->employee . '_' . date('Y-m-d') . '.pdf';
            ...
            return $this->serverErrorResponse('Failed to generate land registration endorsement PDF: ' . $e->getMessage());
        }
    }
}
