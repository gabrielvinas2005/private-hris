...
<head>
    ...
    <title>Monetization Payroll Report</title>
    <style>
...
<body>
    <div class="header">
        <div class="title">Monetization Payroll Report</div>
    </div>
    ...
    <table>
        ...
        <tbody>
            @foreach($monetization as $index => $row)
            <tr>
                <td>{{ $index + 1 }}</td>
                <td>{{ $row->name ?? 'N/A' }}</td>
                <td>{{ $row->employee_no ?? 'N/A' }}</td>
                <td style="text-align: right;">{{ $row->total_days ?? 0 }}</td>
