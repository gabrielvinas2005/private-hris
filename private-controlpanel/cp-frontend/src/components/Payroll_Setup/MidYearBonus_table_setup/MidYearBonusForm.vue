<template>
  <div class="midyear-bonus-form">
    <div class="form-header">
      <h4>Mid Year Bonus Table Configuration</h4>
      <div class="header-actions">
        <el-button type="primary" size="small" @click="addMidYearBonusRow">Add Row</el-button>
      </div>
    </div>
    
    <div class="bonus-table-container">
      <el-table :data="midYearBonusData" border style="width:100%" max-height="500">
        ...
        <el-table-column label="Actions" width="100" align="center">
          <template #default="{ row, $index }">
            <el-button 
              size="small" 
              type="danger" 
              @click="removeMidYearBonusRow($index)"
              :disabled="midYearBonusData.length <= 1"
            >
              Remove
            </el-button>
          </template>
...
function validateMonths(row, index) {
  ...
  const duplicateMonths = props.midYearBonusData.filter((item, idx) => 
...
function handleSave() {
  ...
  const validRows = props.midYearBonusData.filter(row =>
    row.months !== '' && row.percentage !== ''
  )

  if (validRows.length === 0) {
    ElMessage.error('Please fill in at least one mid year bonus row')
    return
  }
