    ...
    <el-row :gutter="16" class="mb-6">
      <el-col :span="6">
        <el-card shadow="hover" class="metric-card">
          <el-statistic title="Total DPCR Records" :value="dpcrRatings.length" />
          <template #suffix>
      ...
      <el-col :span="6">
        <el-card shadow="hover" class="metric-card">
          <el-statistic title="Divisions" :value="divisionCount" />
          <template #suffix>
    ...
    <el-card shadow="never">
      <template #header>
        <div class="flex justify-between items-center">
          <span class="font-medium">DPCR Records</span>
          <el-tag type="info">{{ filteredDPCRRatings.length }} records</el-tag>
        </div>
      </template>

      <el-table 
        v-loading="loading"
        :data="filteredDPCRRatings" 
        border 
        stripe
        :height="tableHeight"
      >
        <el-table-column 
          v-if="visibleColumns.includes('division')"
          prop="division" 
          label="Division" 
          min-width="260"
        >
      ...
      <!-- Empty State -->
      <el-empty 
        v-if="!loading && filteredDPCRRatings.length === 0"
        description="No DPCR records found"
      >
        <el-button type="primary" @click="onAdd">Create First DPCR</el-button>
      </el-empty>
    </el-card>
  </div>
</template>
...
const props = defineProps({
  dpcrRatings: { type: Array, default: () => [] },
...
const visibleColumns = ref(['division', 'period', 'year', 'status'])

// Computed properties
const filteredDPCRRatings = computed(() => {
  if (!searchQuery.value) return props.dpcrRatings
  
  const query = searchQuery.value.toLowerCase()
  return props.dpcrRatings.filter(item => 
...
const divisionCount = computed(() => {
  const divisions = new Set(props.dpcrRatings.map(item => item.division))
  return divisions.size
})

const pendingReviewsCount = computed(() => {
  return props.dpcrRatings.length
})
...
const onPrint = () => {
  try {
    if (filteredDPCRRatings.value.length === 0) {
      ElMessage.warning('No data to print')
      return
    }

    const printContent = generatePrintContent(filteredDPCRRatings.value)
    ...
    ElMessage.error('Failed to print DPCR records')
  }
}
...
const onExcel = async () => {
  try {
    ...
    const exportData = filteredDPCRRatings.value.map(dpcr => ({
      'ID': dpcr.id || '',
      'Division': dpcr.division || '',
      'Year': dpcr.year || '',
      'Month From': dpcr.month_from || '',
      'Month To': dpcr.month_to || '',
      'Status': getStatusText(dpcr)
    }))

    const filename = `dpcr_records_${new Date().toISOString().split('T')[0]}.xlsx`
...
const onPDF = async () => {
  try {
    ...
    const pdfContent = generatePrintContent(filteredDPCRRatings.value)
    const printWindow = window.open('', '_blank')
    if (!printWindow) {
...
const generatePrintContent = (data) => {
  ...
  let tableRows = ''
  data.forEach((dpcr, index) => {
    tableRows += `
      <tr>
        <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${index + 1}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${dpcr.division || 'N/A'}</td>
        <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${dpcr.year || 'N/A'}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${dpcr.month_from || ''}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${dpcr.month_to || ''}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${getStatusText(dpcr)}</td>
      </tr>
    `
  })
  ...
  return `
    ...
    <html>
    <head>
      <title>DPCR Records Report</title>
      <style>
        @media print {
          @page {
            margin: 1cm;
