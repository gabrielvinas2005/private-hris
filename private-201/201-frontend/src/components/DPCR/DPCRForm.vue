      ...
      >
        ...
        <el-row :gutter="16">
          ...
          <el-col :span="12">
            ...
            <el-form-item label="Division" prop="division" required>
        ...
        >
          ...
          <template #default>
            ...
            <p>This will create a DPCR evaluation period for the selected division.</p>

            <ul class="mt-2 ml-4">

              <li>Only the division head from Division Setup will be included</li>

              <li>You can review and rate department heads after creating this DPCR</li>
    ...
    <template #footer>
      ...
      <div class="dialog-footer">
        ...
        >
          ...
          {{ saving ? 'Saving...' : (isEdit ? 'Update DPCR' : 'Create DPCR') }}
...
const rules = {
  ...
  division: [{ required: true, message: 'Please select a division', trigger: 'change' }],
...
const title = computed(() => {
  ...
  return isEdit.value ? 'Edit DPCR' : 'Create New DPCR'
...
const isEdit = computed(() => {
  ...
  return props.dpcrData && props.dpcrData.id > 0
...
watch(() => props.formData?.dpcr_ratings, (dpcrRatings) => {

  if (dpcrRatings && dpcrRatings.length > 0) {

    const dpcrData = dpcrRatings[0]

    if (!props.dpcrData || form.id === 0) {

      form.id = dpcrData.id || 0

      form.division = dpcrData.division_id || 0

      form.month_from = dpcrData.month_from || null

      form.month_to = dpcrData.month_to || null

      form.year = dpcrData.year || new Date().getFullYear()
