<template>
  <PageScaffold title="Acceptance of Resignation" subtitle="Generate acceptance of resignation letter">
        ...
        <el-form ref="formRef" :model="formData" :rules="rules" label-width="180px">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <el-form-item label="Position" prop="position1">
              <el-input v-model="formData.position1" placeholder="Enter signatory position" />
            </el-form-item>
  ...
  <div v-if="showPreview" class="mt-6 bg-white rounded-lg shadow p-4">
    <div class="flex items-center justify-between mb-3">
      <div>
        <div class="text-base font-semibold">Acceptance of Resignation - {{ selectedEmployeeName }}</div>
...
// Inline preview now; modal removed
import { useAcceptanceOfResignation } from '../../composable/useAcceptanceOfResignation.js'

const { loading, generateLoading, employees, fetchEmployees, generateAcceptancePdf, generateAcceptanceWord, previewAcceptanceWord, downloadPDFFromBlob, downloadWordFromBlob } = useAcceptanceOfResignation()
...
const downloadPdf = () => {
  ...
  fetch(pdfUrl.value).then(r => r.blob()).then(blob => {
    downloadPDFFromBlob(blob, `acceptance_of_resignation_${selectedEmployeeName.value.replace(/\s+/g, '_')}.pdf`)
  })
}
...
const downloadWord = async () => {
  try {
    ...
    downloadWordFromBlob(blob, `acceptance_of_resignation_${selectedEmployeeName.value.replace(/\s+/g, '_')}.docx`)
  } catch (error) {
...
const reset = () => {
  formData.value = { employee: '', date: '', effectivity_date: '', signatory: '', position1: '', received_date: '', received_signatory: '' }
