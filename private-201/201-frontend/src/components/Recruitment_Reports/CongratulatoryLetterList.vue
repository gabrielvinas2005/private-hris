<template>
  <PageScaffold title="Congratulatory Letter" subtitle="Generate congratulatory letters for employees">
    ...
    <CertificatePreviewModal
      ...
      :certificate-type="'Congratulatory Letter'"
...
import { useCongratulatoryLetter } from '../../composable/useCongratulatoryLetter.js'
...
const onPreview = async () => {
  ...
  try {
    ...
    const response = await generateCongratulatoryPdf(formData)
...
const downloadPdf = () => {
  if (pdfUrl.value) {
    ...
    link.download = `congratulatory_letter_${formData.employee}_${new Date().toISOString().split('T')[0]}.pdf`
