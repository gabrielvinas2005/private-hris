    ...
    <div v-loading="loading">
      <div v-if="opcrInfo">
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <el-row :gutter="16">
            <el-col :span="12">
              <div class="info-item">
                <label>Office:</label>
                <span>{{ opcrInfo.division }}</span>
              </div>
            </el-col>
            <el-col :span="12">
              <div class="info-item">
                <label>Year:</label>
                <span>{{ opcrInfo.year }}</span>
              </div>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="16">
            <el-col :span="6">
              <div class="info-item">
                ...
                <span>{{ opcrInfo.period_start || 'N/A' }}</span>
              </div>
            </el-col>
            <el-col :span="6">
              <div class="info-item">
                <label>Period To:</label>
                <span>{{ opcrInfo.period_end || 'N/A' }}</span>
              </div>
            </el-col>
          </el-row>
        </el-card>
        ...
        <el-card shadow="never">
          <template #header>
            <div class="flex justify-between items-center">
              <div class="flex items-center">
                ...
                <span>Office Head Performance Ratings</span>
              </div>
              <el-tag type="info">{{ officeHeads.length }} office heads</el-tag>
            </div>
          </template>
          ...
          <el-table 
            :data="filteredOfficeHeads" 
            border 
            stripe
            :height="tableHeight"
            class="rating-table"
          >
            <el-table-column label="Office Head" min-width="260">
          ...
          <el-empty 
            v-if="!loading && filteredOfficeHeads.length === 0"
            description="No office heads found for this OPCR"
          />
        </el-card>
      </div>
    </div>
  ...
  <el-drawer v-model="previewVisible" title="OPCR Preview" size="60%">
    <div v-if="selectedRow">
      ...
      <el-table :data="selectedRow.outputs || []" size="small" border>
        <!-- Use category from employee_opcr_outputs instead of a separate Output field -->
      ...
      <div v-for="level in recalibrationLevels" :key="level.key" class="mb-4">
        ...
        <el-table :data="groupedRecalibrations(selectedRow)[level.key] || []" size="small" border>
          <!-- Match layout with Outputs table (using category) -->
          <el-table-column prop="category" label="Category" min-width="200" />
          <el-table-column prop="success_indicators" label="Success Indicators" min-width="200" />
          <el-table-column prop="actual_accomplishments" label="Accomplishment" min-width="200" />
          <el-table-column prop="quality_rating" label="Quality" width="90" />
...
const opcrInfo = computed(() => {
  return props.reviewData.length > 0 ? props.reviewData[0] : null
})

const title = computed(() => {
  const info = opcrInfo.value
  if (info) {
    return `Office Head Review - ${info.department || 'N/A'}`
  }
  return 'Office Head Performance Review'
})

const filteredOfficeHeads = computed(() => {
  if (!searchQuery.value) return officeHeads.value
  
  const query = searchQuery.value.toLowerCase()
  return officeHeads.value.filter(emp => 
...
watch(() => props.reviewData, (newData) => {
  if (newData && newData.length > 0) {
    officeHeads.value = newData.map(emp => ({
      ...emp,
