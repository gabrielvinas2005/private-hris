    ...
    <el-row :gutter="16" class="mb-6">
      <el-col :span="6">
        <el-card shadow="hover" class="metric-card">
          <el-statistic title="Total OPCR Records" :value="opcrRatings.length" />
          <template #suffix>
    ...
    <el-card shadow="never">
      <template #header>
        <div class="flex justify-between items-center">
          <span class="font-medium">OPCR Records</span>
          <el-tag type="info">{{ filteredOPCRRatings.length }} records</el-tag>
        </div>
      </template>

      <el-table 
        v-loading="loading"
        :data="filteredOPCRRatings" 
        border 
        stripe
        :height="tableHeight"
      >
        <el-table-column 
          v-if="visibleColumns.includes('office')"
          prop="division" 
          label="Office" 
          min-width="260"
        >
          <template #default="{ row }">
            <div class="font-medium">{{ row.division }}</div>
          </template>
        </el-table-column>
      ...
      <!-- Empty State -->
      <el-empty 
        v-if="!loading && filteredOPCRRatings.length === 0"
        description="No OPCR records found"
      >
        <el-button type="primary" @click="onAdd">Create First OPCR</el-button>
      </el-empty>
    </el-card>
  </div>
</template>
...
const props = defineProps({
  opcrRatings: { type: Array, default: () => [] },
...
const visibleColumns = ref(['office', 'period', 'year', 'status'])

// Computed properties
const filteredOPCRRatings = computed(() => {
  if (!searchQuery.value) return props.opcrRatings
  
  const query = searchQuery.value.toLowerCase()
  return props.opcrRatings.filter(item => 
...
const departmentCount = computed(() => {
  const offices = new Set(props.opcrRatings.map(item => item.division))
  return offices.size
})

const pendingReviewsCount = computed(() => {
  return props.opcrRatings.length
})
...
const onPrint = () => {
  try {
    if (filteredOPCRRatings.value.length === 0) {
      ElMessage.warning('No data to print')
      return
    }

    const printContent = generatePrintContent(filteredOPCRRatings.value)
    ...
    ElMessage.error('Failed to print OPCR records')
  }
}
...
const onExcel = async () => {
  try {
    ...
    const exportData = filteredOPCRRatings.value.map(opcr => ({
      'ID': opcr.id || '',
      'Office': opcr.division || '',
      'Year': opcr.year || '',
      'Month From': opcr.month_from || '',
      'Month To': opcr.month_to || '',
      'Status': getStatusText(opcr)
    }))

    const filename = `opcr_records_${new Date().toISOString().split('T')[0]}.xlsx`
...
const onPDF = async () => {
  try {
    ...
    const pdfContent = generatePrintContent(filteredOPCRRatings.value)
    const printWindow = window.open('', '_blank')
    if (!printWindow) {
...
const generatePrintContent = (data) => {
  ...
  let tableRows = ''
  data.forEach((opcr, index) => {
    tableRows += `
      <tr>
        <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${index + 1}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${opcr.department || ''}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${opcr.division || 'N/A'}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${opcr.section || ''}</td>
        <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${opcr.year || 'N/A'}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${opcr.month_from || ''}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${opcr.month_to || ''}</td>
        <td style="border: 1px solid #ddd; padding: 8px;">${getStatusText(opcr)}</td>
      </tr>
    `
  })
