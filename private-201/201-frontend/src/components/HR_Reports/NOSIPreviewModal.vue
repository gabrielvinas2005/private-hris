<template>
  <el-dialog
    v-model="visible"
    :title="`Salary Step Increment - ${employeeName}`"
    width="90%"
    :before-close="handleClose"
    class="nosi-preview-modal"
  >
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <h3 class="text-lg font-semibold text-gray-900">NOSI Preview</h3>
    ...
    <div class="pdf-container">
      <div v-if="loading" class="flex items-center justify-center h-96">
        ...
        <div class="ml-3">
          <p class="text-gray-600">Generating NOSI preview...</p>
...
const props = defineProps({
  ...
  filename: {
    type: String,
    default: 'notice_of_salary_step.pdf'
  },
  employeeName: {
    type: String,
    default: 'Employee'
...
const handleDownload = async () => {
  ...
  try {
    ...
    ElMessage.success('NOSI PDF downloaded successfully')
  } catch (error) {
...
.nosi-preview-modal :deep(.el-dialog) {
...
.nosi-preview-modal :deep(.el-dialog__body) {
  flex: 1;
  padding: 0;
  overflow: hidden;
}
