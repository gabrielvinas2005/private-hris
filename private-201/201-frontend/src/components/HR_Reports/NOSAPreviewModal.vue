<template>
  <el-dialog
    v-model="visible"
    :title="`Salary Adjustment - ${employeeName}`"
    width="90%"
    :before-close="handleClose"
    class="nosa-preview-modal"
  >
    <template #header>
      <div class="flex items-center justify-between">
        <div class="flex items-center space-x-3">
          ...
          <div>
            <h3 class="text-lg font-semibold text-gray-900">NOSA Preview</h3>
    ...
    <div class="pdf-container">
      <div v-if="loading" class="flex items-center justify-center h-96">
        ...
        <div class="ml-3">
          <p class="text-gray-600">Generating NOSA preview...</p>
...
const props = defineProps({
  ...
  filename: {
    type: String,
    default: 'notice_of_salary_adjustment.pdf'
  },
  employeeName: {
    type: String,
    default: 'Employee'
...
const handleDownload = async () => {
  ...
  try {
    ...
    ElMessage.success('NOSA PDF downloaded successfully')
  } catch (error) {
...
.nosa-preview-modal :deep(.el-dialog) {
...
.nosa-preview-modal :deep(.el-dialog__body) {
  flex: 1;
  padding: 0;
  overflow: hidden;
}
