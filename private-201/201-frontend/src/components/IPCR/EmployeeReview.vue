<template>
      ...
      <div v-if="ipcrInfo">
        ...
        <el-card shadow="never" class="mb-4">
          ...
          <el-row :gutter="16">
            <el-col :span="6">
              <div class="info-item">
                <label>Section:</label>
                <span>{{ ipcrInfo.section }}</span>
              </div>
            </el-col>
            <el-col :span="6">
              <div class="info-item">
                <label>Year:</label>
                <span>{{ ipcrInfo.year }}</span>
              </div>
            </el-col>
          </el-row>
          ...
          <el-row :gutter="16">
            <el-col :span="6">
              <div class="info-item">
                ...
                <span>{{ ipcrInfo.period_start || 'N/A' }}</span>
              </div>
            </el-col>
            <el-col :span="6">
              <div class="info-item">
                <label>Period To:</label>
                <span>{{ ipcrInfo.period_end || 'N/A' }}</span>
              </div>
            </el-col>
          </el-row>
        </el-card>
        ...
        <el-card shadow="never">
          <template #header>
            <div class="flex justify-between items-center">
              <div class="flex items-center">
                ...
                <span>Employee Performance Ratings</span>
              </div>
              <div class="flex items-center gap-2">
                ...
                <el-tag type="info">{{ employees.length }} employees</el-tag>
              </div>
            </div>
          </template>
          ...
          <el-table 
            ...
            class="rating-table"
          >
            <el-table-column label="Employee" min-width="260">
          ...
          <el-empty 
            ...
            description="No employees found for this IPCR"
          />
        </el-card>
      </div>
    </div>
  ...
  <el-drawer v-model="previewVisible" title="IPCR Preview" size="60%">
    <div v-if="selectedRow">
      ...
      <h4 class="mt-4 mb-2">Self & Supervisor Average</h4>
      <p class="text-sm text-gray-500 mb-2">
        Quality, Efficiency, and Timeliness are averaged from self-assessment and supervisor calibration per output.
      </p>
      <el-table :data="selfSupervisorAverages" size="small" border>
        <el-table-column prop="output" label="Output" min-width="200" />
        <el-table-column label="Quality (Q)" width="100" align="center">
          <template #default="{ row }">
            {{ formatRating(row.quality_rating) }}
          </template>
        </el-table-column>
        <el-table-column label="Efficiency (E)" width="110" align="center">
          <template #default="{ row }">
            {{ formatRating(row.efficiency_rating) }}
          </template>
        </el-table-column>
        <el-table-column label="Timeliness (T)" width="120" align="center">
          <template #default="{ row }">
            {{ formatRating(row.timeliness_rating) }}
          </template>
        </el-table-column>
        <el-table-column label="Average" width="90" align="center">
          <template #default="{ row }">
            {{ formatRating(row.average_rating) }}
          </template>
        </el-table-column>
      </el-table>
      <el-empty v-if="selfSupervisorAverages.length === 0" description="No self and supervisor ratings to average" />
...
const ipcrInfo = computed(() => {
  return props.reviewData.length > 0 ? props.reviewData[0] : null
})

const title = computed(() => {
  const info = ipcrInfo.value
  if (info) {
    return `Employee Review - ${info.section || 'N/A'}`
  }
...
const averageRatings = (first, second) => {
  const values = [toRatingNumber(first), toRatingNumber(second)].filter(v => v !== null)
  if (values.length === 0) return null
  return values.reduce((sum, value) => sum + value, 0) / values.length
}

const getLatestRecalibration = (output, level) => {
  const recs = (output.recalibrations || []).filter(rec => rec.recalibration_level === level)
  if (!recs.length) return null
  return recs.sort((a, b) => (b.id || 0) - (a.id || 0))[0]
}

const buildSelfSupervisorAverages = (row) => {
  if (!row?.outputs?.length) return []

  return row.outputs.map((output) => {
    const supervisor = getLatestRecalibration(output, 'supervisor')
    const quality = averageRatings(output.quality_rating, supervisor?.quality_rating)
    const efficiency = averageRatings(output.efficiency_rating, supervisor?.efficiency_rating)
    const timeliness = averageRatings(output.timeliness_rating, supervisor?.timeliness_rating)
    const components = [quality, efficiency, timeliness].filter(v => v !== null)
    const average = components.length
...
const formatRating = (value) => {
  const num = toRatingNumber(value)
  return num === null ? 'N/A' : num.toFixed(2)
}

const selfSupervisorAverages = computed(() => buildSelfSupervisorAverages(selectedRow.value))

const summaryRows = computed(() => {
  return employees.value.map((emp, idx) => {
    const outputs = emp.outputs || []
    const ratings = outputs
      .map(o => Number(o.average_rating))
    ...
    return {
      ...
      section: emp.section || '',
      performance: avg !== null ? avg.toFixed(2) : 'N/A'
    }
  })
})
...
const downloadSummary = async () => {
  const info = ipcrInfo.value
  const id = info?.ipcr_header_id || info?.id
  if (!id) {
    ElMessage.error('Missing IPCR reference')
    return
  }
  try {
    summaryLoading.value = true
    const res = await ipcrApi.summaryRatings(id)
...
const openInNewTab = () => {
  if (summaryUrl.value) {
    window.open(summaryUrl.value, '_blank')
  }
}
...
const downloadPmt = async () => {
  ...
  try {
    ...
    const res = await ipcrApi.pmtCalibration(id)
    const blob = new Blob([res.data], { type: 'application/pdf' })
    const url = window.URL.createObjectURL(blob)
    pmtUrl.value = url
  } catch (err) {
...
watch(() => props.reviewData, (newData) => {
  if (newData && newData.length > 0) {
    employees.value = newData.map(emp => ({
      ...emp,
...
.adjectival-input .el-input__inner {
