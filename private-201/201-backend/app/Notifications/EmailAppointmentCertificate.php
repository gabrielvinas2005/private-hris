...
class EmailAppointmentCertificate extends Notification
{
    use Queueable;
    ...
    public function toMail($notifiable)
    {
        ...
        $mail = (new MailMessage)
            ->subject('Appointment Certificate')
            ->greeting('Dear ' . $name . ',')
            ->line('Please find your appointment certificate attached to this email.')
            ->line('For any concerns, please contact HR.')
            ->salutation('Best regards, ' . BranchHelper::getMainBranchCode() . ' Team');

        if ($this->pdfPath && file_exists($this->pdfPath)) {
            $mail->attach($this->pdfPath, [
                'as' => 'Appointment_Certificate.pdf',
