...
class StepIncrementController extends Controller
{
            ...
            if (Auth::user()->access_all_branches) {
                $data = DB::table('step_increments as a')
                    ...
                    ->select(
                        ...
                        'c.name as month',
                        'a.year_id as year',
                        DB::raw('ISNULL(a.is_forwarded, 0) as is_forwarded'),
                        DB::raw('ISNULL(a.is_approved, 0) as is_approved'),
                        DB::raw('ISNULL(a.is_disapproved, 0) as is_disapproved'),
                        'a.forwarded_date',
                        'a.approved_date',
                        'a.approved_by_id',
        ...
        try {
            ...
            for ($i = 0; $i < $arr_len; $i++) {
                if ($data['employee_id'][$i] != NULL) {
                    ...
                    for ($i = 0; $i < $arr_len; $i++) {
                        if ($amount >= $tax_data[$i]->min_amount && $amount <= $tax_data[$i]->max_amount) {
                            $tax_amount = ((($amount - $tax_data[$i]->min_amount) * $tax_data[$i]->percentage) + $tax_data[$i]->base_tax);
                            break;
                        }
                    }
    ...
    public function forward($id)
    {
        try {
            ...
            DB::table('step_increments')->where('id', $id)->update([
                'is_forwarded' => true,
                'forwarded_date' => now()
            ]);
            ...
            $data_audit = array(
                ...
                'menu'    => 'Step Increment',
                'activity' => 'Forward',
                'description' => 'Forwarded step increment for approval.',
            );

            Audit::create($data_audit);

            return $this->successResponse(null, 'Step increment forwarded successfully');
        } catch (\Exception $e) {
            return $this->serverErrorResponse('Failed to forward step increment: ' . $e->getMessage());
        }
    }
    ...
    public function approve($id)
    {
        try {
            ...
            $data_audit = array(
                ...
                'menu'    => 'Step Increment',
                'activity' => 'Approve',
                'description' => 'Approved step increment.',
            );
    ...
    public function reject($id)
    {
        try {
            ...
            $data_audit = array(
                ...
                'menu'    => 'Step Increment',
                'activity' => 'Reject',
                'description' => 'Rejected step increment.',
            );
