...
class OffBoardingTypeController extends Controller
{
    ...
    {
        try {
            $offboarding = DB::table('employee_offboardings')
                ->select('id', 'employee_id')
                ->where('id', $id)
                ->first();

            if (!$offboarding) {
                return $this->notFoundResponse('Off-boarding record not found.');
            }

            DB::beginTransaction();

            // Mark off-boarding record as rehired/reactivated
            DB::table('employee_offboardings')
                ->where('id', $id)
                ->update(['reactivated_status_id' => 1]);

            // Reactivate employee so they show again in Employee Records
            DB::table('employees')
            ...
            // Unlock linked user account (if any)
            ...
            if ($user) {
                DB::table('users')
                    ->where('id', $user->id)
                    ->update(['locked' => false, 'locked_date' => null]);
            }
            ...
            Audit::create([
                ...
                'menu'    => 'Off-boarding',
                'activity' => 'Rehire',
                'description' => 'Rehired employee from off-boarding record ID ' . $id . '.',
            ]);
            ...
            ], 'Successfully rehired!');
        } catch (\Exception $e) {
            DB::rollBack();
            return $this->serverErrorResponse('Failed to rehire off-boarding type: ' . $e->getMessage());
        }
    }
    public function destroy($id)
    {
        try {
            ...
            $data_audit = array(
                ...
                'menu'    => 'Off-Boarding Types Setup',
                'activity' => 'Delete',
                'description' => 'Deleted off-boarding informations.',
            );
            Audit::create($data_audit);
