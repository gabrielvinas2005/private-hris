...
use PhpOffice\PhpWord\PhpWord;                    // Add this line
use PhpOffice\PhpWord\IOFactory;                  // Add this if you use IOFactory
...
class AcceptanceofResignationController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $data = DB::table('employees')
                ...
                ->where('employee_offboardings.nature_id', 1) // Only Resignation (nature_id = 1)
                ->paginate(10000);
    ...
    public function generatePdf(Request $request)
    {
        try {
            ...
            $app_key = env("APP_KEY", "");
            // One row per employee: ipcr / offboarding joins multiply rows when history has multiple records
            $ipcrLatest = DB::table('ipcr')
                ->select('employee_id', DB::raw('MAX(id) as latest_ipcr_id'))
                ->groupBy('employee_id');

            $offboardingLatest = DB::table('employee_offboardings')
                ->select('employee_id', DB::raw('MAX(id) as latest_offboarding_id'))
                ->where('nature_id', 1)
                ->where('reactivated_status_id', 0)
                ->groupBy('employee_id');

            $employees = DB::table('employees as a')
                ->leftJoinSub($offboardingLatest, 'b_max', function ($join) {
                    $join->on('a.id', '=', 'b_max.employee_id');
                })
                ->leftJoin('employee_offboardings as b', 'b.id', '=', 'b_max.latest_offboarding_id')
                ->leftJoin('positions as c', 'a.position_id', '=', 'c.id')
                ->leftJoin('divisions as dv', 'a.division_id', '=', 'dv.id')
                ->leftJoin('name_prefixes as e', 'a.name_prefix_id', '=', 'e.id')
                ->leftJoinSub($ipcrLatest, 'i_max', function ($join) {
                    $join->on('a.id', '=', 'i_max.employee_id');
                })
                ->leftJoin('ipcr as i', 'i.id', '=', 'i_max.latest_ipcr_id')
    ...
    public function generateWord(Request $request)
{
    try {
        ...
        $employees = DB::table('employees as a')
            ...
            ->leftJoin('companies as cp', 'cp.id', '=', 'a.company_id')
            ->select(
                'a.id', 'a.ra_region', 'a.ra_province', 'a.ra_city', 'a.ra_barangay',
                'a.pa_region', 'a.pa_province', 'a.pa_city', 'a.pa_barangay',
                'a.pa_house_no', 'a.pa_street', 'a.pa_village',
                DB::raw("CASE WHEN ISNULL(a.is_encrypted,0) = 0 THEN a.first_name ELSE dbo.ufn_DecryptString(a.first_name,'$app_key') END as first_name"),
                DB::raw("CASE WHEN ISNULL(a.is_encrypted,0) = 0 THEN a.middle_name ELSE dbo.ufn_DecryptString(a.middle_name,'$app_key') END as middle_name"),
                DB::raw("CASE WHEN ISNULL(a.is_encrypted,0) = 0 THEN a.last_name ELSE dbo.ufn_DecryptString(a.last_name,'$app_key') END as last_name"),
                'b.date_effectivity', 'c.name as position', 'dv.name as division', 'i.adjectival_rating',
                'e.name as name_prefix',
                DB::raw("CASE WHEN ISNULL(a.is_encrypted,0) = 0 THEN CONCAT(a.first_name,' ',a.last_name) ELSE RTRIM([dbo].[ufn_DecryptString](a.first_name,'$app_key'))+' '+RTRIM([dbo].[ufn_DecryptString](a.last_name,'$app_key')) END as name"),
                'cp.name as company'
            )
            ->where('a.id', $request->employee)
            ->orderBy('first_name', 'asc')
        ...
        $filename = 'acceptance_of_resignation_' . $request->employee . '_' . date('Y-m-d') . '.docx';
        $tempFile = tempnam(sys_get_temp_dir(), 'acceptance_resignation_');
        ...
        return $this->serverErrorResponse('Failed to generate acceptance of resignation Word document: ' . $e->getMessage());
    }
}
...
private function createAcceptanceWordDocument($employee, $signatories, $companies, $address)
{
    ...
    // 3. Title - centered
    $section->addText('ACCEPTANCE OF RESIGNATION', ['bold' => true, 'size' => 12, 'name' => 'Arial', 'color' => '000000'], $centerAlign);
    $section->addTextBreak(1);
    ...
    // 7. Salutation
        ...
        // 8. First paragraph - justified with first-line indent (manual NBSP padding to force visible indent)
        $paragraph1 = $section->addTextRun(array_merge($justifyAlign, ['indentation' => ['firstLine' => 720]]));
        $paragraph1->addText(str_repeat(chr(160), 8));
    ...
    $paragraph1->addText(' tendering your resignation from the position of ', $normalStyle);
    $paragraph1->addText(strtoupper($employee->position ?? 'COA-AUDITOR'), $underlineStyle);
    $paragraph1->addText(' in ', $normalStyle);
    $paragraph1->addText(strtoupper($employee->division ?? ''), $underlineStyle);
    ...
    $section->addTextBreak(2);
        ...
        // 9. Second paragraph - justified with first-line indent (manual NBSP padding to force visible indent)
        $paragraph2 = $section->addTextRun(array_merge($justifyAlign, ['indentation' => ['firstLine' => 720]]));
        $paragraph2->addText(str_repeat(chr(160), 8));
    ...
    // 10-12. Two-column footer (left: acknowledgement, right: signatory)
    // add extra spacing before the footer block
    $section->addTextBreak(2);
...
public function previewWord(Request $request)
{
    try {
        ...
        // Data preparation with full select (matches generateWord)
        $app_key = env("APP_KEY", "");
        $ipcrLatest = DB::table('ipcr')
            ->select('employee_id', DB::raw('MAX(id) as latest_ipcr_id'))
            ->groupBy('employee_id');
        ...
        $filename = 'acceptance_of_resignation_preview_' . $request->employee . '_' . date('Y-m-d') . '.docx';
        $tempFile = tempnam(sys_get_temp_dir(), 'acceptance_resignation_');
    ...
    /**
     * Clean HTML for PhpWord compatibility
     */
    private function cleanHtmlForWord($html)
    {
        ...
        // Remove unsupported attributes and styles
        $html = preg_replace('/\s(class|id|onclick|style)="[^"]*"/i', '', $html);
        
        // Convert some tags to supported equivalents
        $html = str_replace(['<i>', '</i>'], ['<em>', '</em>'], $html);
        ...
        $html = strip_tags($html, '<div><p><strong><u><em><br><span>');
        
        // Remove empty elements
        $html = preg_replace('/<(p|div|span)>\s*<\/\1>/', '', $html);
