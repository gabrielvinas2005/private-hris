        ...
        if ($employees->isEmpty()) {
            ...
            foreach ($employees as $emp) {
                ...
                if ($employee_tax_adjustments->isNotEmpty()) {
                    ...
                    // Use basic salary + taxable income directly, not after subtracting deductions
                    $tax_data = DB::table('tax_tables')->orderBy('min_amount')->get();
                    $arr_len = DB::table('tax_tables')->count('id');

                    $tax_base_amount = $salary + $total_taxable_income;
                    $tax_amount = 0;

                    for ($i = 0; $i < $arr_len; $i++) {
                        if ($tax_base_amount >= $tax_data[$i]->min_amount && $tax_base_amount <= $tax_data[$i]->max_amount) {
                            $tax_amount = ((($tax_base_amount - $tax_data[$i]->min_amount) * $tax_data[$i]->percentage) + $tax_data[$i]->base_tax);
                            break;
                        }
                    }
                }
