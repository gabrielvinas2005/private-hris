                ...
                $info = DB::table('users as a')
                    ...
                    ->select(
                        ...
                        DB::raw("CASE WHEN ISNULL(b.is_encrypted,0) = 0 THEN
                                   ISNULL(c.name,'')+' '+ISNULL(b.first_name,'')+' '+ISNULL(substring(b.middle_name,1,1),'')+'. '+ISNULL(b.last_name,'')+' '+ISNULL(d.name,'')
                                ELSE
                                   ISNULL(c.name,'')+' '+RTRIM(ISNULL(dbo.ufn_DecryptString(b.first_name,'$app_key'),''))+' '+UPPER(substring(ISNULL(dbo.ufn_DecryptString(b.middle_name,'$app_key'),''),1,1))+'. '+RTRIM(ISNULL(dbo.ufn_DecryptString(b.last_name,'$app_key'),''))+' '+ISNULL(d.name,'') 
                                END as name"),
                        'b.birth_place',
                        'b.birthdate',
                        'b.age',
                        ...
                        DB::raw("CASE WHEN ISNULL(b.is_encrypted,0) = 0 THEN ISNULL(b.email,'') ELSE ISNULL(dbo.ufn_DecryptString(b.email,'$app_key'),'') END as email"),
                        DB::raw("CASE WHEN ISNULL(b.is_encrypted,0) = 0 THEN ISNULL(b.mobile_no,'') ELSE ISNULL(dbo.ufn_DecryptString(b.mobile_no,'$app_key'),'') END as mobile_no"),
                        DB::raw("CASE WHEN ISNULL(b.is_encrypted,0) = 0 THEN ISNULL(b.telephone_no,'') ELSE ISNULL(dbo.ufn_DecryptString(b.telephone_no,'$app_key'),'') END as telephone_no"),
        ...
        try {
            ...
            if (Auth::user()->access_all_branches) {
                $info = DB::table('employees as b')
                    ...
                    ->leftJoin('salary_grades as r', 'r.id', '=', 'b.salary_grade_id')
                    ->leftJoin('salary_steps as q', 'q.id', '=', 'b.salary_step_id')
                    ->leftJoin('fix_schedules as fs', 'fs.id', '=', 'b.work_schedule_id')
                    ->leftJoin('shift_schedules_headers as ssh', 'ssh.id', '=', 'b.work_schedule_id')
                    ->select(
                        'b.employee_no',
                        'b.access_no',
                        'b.photo',
                        DB::raw("CASE WHEN ISNULL(b.is_encrypted,0) = 0 THEN
                                   ISNULL(b.last_name,'') + ', ' + ISNULL(b.first_name,'') +
                                   CASE WHEN ISNULL(b.middle_name,'') <> '' THEN ' ' + UPPER(SUBSTRING(b.middle_name,1,1)) + '.' ELSE '' END
                                ELSE
                                   ISNULL(dbo.ufn_DecryptString(b.last_name,'$app_key'),'') + ', ' +
                                   ISNULL(dbo.ufn_DecryptString(b.first_name,'$app_key'),'') +
                                   CASE WHEN ISNULL(dbo.ufn_DecryptString(b.middle_name,'$app_key'),'') <> '' 
                                        THEN ' ' + UPPER(SUBSTRING(dbo.ufn_DecryptString(b.middle_name,'$app_key'),1,1)) + '.' 
                                        ELSE '' END
                                END as name"),
                        'b.birth_place',
                        'b.birthdate',
                        ...
                        'k.name as branch',
                        'l.name as department',
                        DB::raw("CASE WHEN ISNULL(b.is_shifting,0) = 0 THEN ISNULL(fs.name,'') ELSE ISNULL(ssh.name,'') END as work_schedule"),
                        'm.name as employment_type',
                        'n.name as position',
                        'o.code as plantilla',
                        'r.name as grade',
            ...
            // If Excel format is requested, generate XLSX using PhpSpreadsheet
            if ($format === 'xlsx') {
                if ($info->isEmpty()) {
                    return $this->serverErrorResponse('No employee data available for export.');
                }
                ...
                // Title row
                $sheet->setCellValue('A1', 'Employee Data Export');
                $sheet->mergeCells('A1:D1');
                $sheet->getStyle('A1')->getFont()->setBold(true)->setSize(14);

                // Info row
                $sheet->setCellValue('A2', 'Generated on: ' . now()->format('Y-m-d H:i'));
                $sheet->setCellValue('B2', 'Total records: ' . $info->count());

                // Headers (remove internal ID and order columns)
                $firstRow = (array) $info->first();
                $headers = array_keys($firstRow);
                // Remove raw ID column if present
                $headers = array_values(array_filter($headers, function ($h) {
                    return $h !== 'id';
                }));

                // Desired leading order: Name, Photo, Employee No, Access No
                $preferredOrder = ['name', 'photo', 'employee_no', 'access_no'];
                $orderedHeaders = [];
                ...
                foreach ($headers as $h) {
                    if (!in_array($h, $orderedHeaders, true)) {
                        $orderedHeaders[] = $h;
                    }
                }
                ...
                foreach ($headers as $index => $header) {
                    ...
                    $label = str_replace('_', ' ', ucfirst($header));
                ...
                $photoFieldRegex = '/photo|avatar|image/i';
                ...
                foreach ($info as $row) {
                    foreach ($headers as $index => $field) {
                        ...
                        // Boolean normalization (1/0 and true/false -> Y/N)
                        ...
                        // Photo/image handling for binary/base64 photo column
                        if (preg_match($photoFieldRegex, $field) && !empty($value)) {
                            if (is_string($value)) {
                                $base64 = $value;
                                ...
                                if ($imageData !== false) {
                                    try {
                                        $tmpPath = storage_path('app/tmp_photo_' . uniqid() . '.jpg');
                                        ...
                                        $drawing->setCoordinates($col . $rowIndex);
                                        $drawing->setWorksheet($sheet);

                                        // Adjust column width and row height for image
                                        $sheet->getColumnDimension($col)->setWidth(14);
                                        $sheet->getRowDimension($rowIndex)->setRowHeight(45);

                                        continue;
                                    } catch (\Exception $e) {
                                        // If image embedding fails, fall back to writing raw value
                                    }
                                }
                            }
                        }
                ...
                $fileName = 'employee_data_' . now()->format('Ymd_His') . '.xlsx';
