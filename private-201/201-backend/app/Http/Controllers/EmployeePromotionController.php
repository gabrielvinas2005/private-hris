...
class EmployeePromotionController extends Controller
{
    ...
    public function add($id)
    {
        ...
        // Get Plantilla - show unassigned plantillas and plantillas for any current employees (since we need to show current assignments)
        ...
        if (Auth::user()->access_all_branches) {
            ...
            $employees = DB::table('employees')
                ...
                ->where([
                    'is_employee' => true,
                    'active' => true,
                    // Intentionally not filtering by branch_id:
                    // promotion employee dropdown should not be restricted by branch.
                ])
                ->orderBy('first_name', 'asc')
                ->get();
        }
    ...
    public function store(Request $request, $id)
    {
        ...
        if ($request->has('is_plantilla')) {
            ...
            // Update the selected plantilla assignment and also keep it consistent
            // with any changes to salary grade/step/position done in the edit modal.
            $plantilla_data = array(
                'employee_id' => $employee_id,
            );

            if ($request->has('position_id') && !empty($request->position_id)) {
                $plantilla_data['position_id'] = $request->position_id;
            }
            if ($request->has('salary_grade_id') && !empty($request->salary_grade_id)) {
                $plantilla_data['salary_grade_id'] = $request->salary_grade_id;
            }
            if ($request->has('salary_step_id') && !empty($request->salary_step_id)) {
                $plantilla_data['salary_step_id'] = $request->salary_step_id;
            }
        ...
        if ($id == 0) {
            $data_audit = array(
                ...
                'menu'    => 'Promotion',
                'activity' => 'Add',
                'description' => 'Added employee promotion informations.',
            );
            ...
            $data_audit = array(
                ...
                'menu'    => 'Promotion',
                'activity' => 'Update',
                'description' => 'Updated employee promotion informations.',
            );
    ...
    public function show($id)
    {
        try {
            ...
            $promotion = DB::table('employee_promotions as a')
                ...
                ->leftJoin('plantillas as i', 'a.plantilla_id', '=', 'i.id')
                ->leftJoin('salary_grades as j', 'i.salary_grade_id', '=', 'j.id')
                ->leftJoin('salary_steps as k', 'i.salary_step_id', '=', 'k.id')
                ->select(
                    'a.*',
                    ...
                    'i.code as plantilla_code',
                    'j.name as salary_grade_name',
                    'k.name as salary_step_name'
                )
                ->where('a.id', $id)
                ->first();
            ...
            return $this->successResponse([
                ...
                'summary' => [
                    ...
                    'payroll_interval_name' => $promotion->payroll_interval_name,
                    'plantilla_code' => $promotion->plantilla_code,
                    'salary_grade_name' => $promotion->salary_grade_name,
                    'salary_step_name' => $promotion->salary_step_name,
                    'date_of_effectivity' => $promotion->date_of_effectivity,
                    'date_position_appointed' => $promotion->date_position_appointed,
                    'is_plantilla' => $promotion->is_plantilla,
                    'old_salary' => $promotion->old_salary,
                    'new_salary' => $promotion->new_salary,
                    'old_tax_amount' => $promotion->old_tax_amount,
                    'old_gsis_amount' => $promotion->old_gsis_amount,
                    'old_sss_amount' => $promotion->old_sss_amount,
                    'old_pagibig_amount' => $promotion->old_pagibig_amount,
                    'old_philhealth_amount' => $promotion->old_philhealth_amount,
                    'new_tax_amount' => $promotion->new_tax_amount,
                    'new_gsis_amount' => $promotion->new_gsis_amount,
                    'new_sss_amount' => $promotion->new_sss_amount,
                    'new_pagibig_amount' => $promotion->new_pagibig_amount,
                    'new_philhealth_amount' => $promotion->new_philhealth_amount,
