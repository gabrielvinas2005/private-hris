...
<head>
    ...
    <style>
        * {
        ...
        .field-list {
            list-style: none;
            padding-left: 30px; /* indent bullets for Duration–Summary section */
        }
        ...
        .nested-value-list {
            list-style-type: circle; /* hollow round bullet for values */
            margin: 5px 0 0 20px;
            padding-left: 0;
        }
...
<body>
            ...
            <div class="work-experiences-container">
                @foreach($work_experiences as $index => $experience)
                    ...
                    <div class="work-section-column">
                        @if($loop->first)
                            <div class="sample-note">Sample: If applying to Supervising Administrative Officer (Human Resource Management Officer IV)</div>
                        @endif
                        <ul class="field-list">
                            <li class="field-item">
                                <span class="field-label">Duration:</span> {{ $experience->duration_inclusion_display ?? ($experience->Duration ?? '') }}
                            </li>
                            <li class="field-item">
                                <span class="field-label">Position:</span> {{ $experience->Position ?? '' }}
                            </li>
                            <li class="field-item">
                                <span class="field-label">Name of Office/Unit:</span> {{ $experience->Office_name ?? '' }}
                            </li>
                            <li class="field-item">
                                <span class="field-label">Immediate Supervisor:</span> {{ $experience->Immediate_supervisor ?? '' }}
                            </li>
                            <li class="field-item">
                                <span class="field-label">Name of Agency/Organization and Location:</span> {{ $experience->Office_Address ?? '' }}
                            </li>
                        </ul>
    ...
    <div class="signature-section">
        <div class="signature-line">{{ isset($work_experiences[0]) ? $work_experiences[0]->applicant_name ?? '' : '' }}</div>
        <div class="signature-text">(Signature over Printed Name</div>
        <div class="signature-text">of Employee/Applicant)</div>

        <div class="date-section">
            <span class="signature-text">Date:<u>{{ date('F d, Y') }}</u></span>
        </div>
    </div>
</body>
</html>
