...
<head>
    ...
    <title>Acceptance of Resignation</title>
            ...
            <div style="margin-left: 1.5in;margin-right: 1.5in;">
                ...
                <div>
                    <p style="padding-top: 40px;padding-bottom: 20px;text-align: center; font: 18px, arial;">
                        ACCEPTANCE OF RESIGNATION</p>
                </div>
                <div style="text-align: right;">
                    <p style="padding-top: 20px;padding-bottom: 20px;font: 14px, arial;">
                        {{ date('F d, Y', strtotime(now())) }}
                    </p>
                </div>
                <div style="text-align:left; font: 14px, arial;">
                    <p><b>{{ strtoupper($resignation->name_prefix) }} {{ strtoupper($resignation->name) }} </></b>
                    </p>
                    <p>{{ $employees[0]->pa_house_no == null ? '' : $employees[0]->pa_house_no . ', ' }}{{ $employees[0]->pa_village == null ? '' : $employees[0]->pa_village . ', ' }}{{ $employees[0]->pa_street == null ? '' : $employees[0]->pa_street . ', ' }}{{ 'Brgy. ' . $address['pa_brgy'] }},
                                {{ $address['pa_city'] }}, {{ $address['pa_province'] }},
                                {{ $address['pa_region'] }}</p><br>
                </div>
                <div>
                    <div>
                        <p style="text-align:left; font: 14px, arial;">
                            @php
                                $rawGender = strtolower((string) ($resignation->gender ?? ''));
                                $isFemale = ($resignation->gender_id ?? null) == 1 || $rawGender === 'female';
                                $title = $isFemale ? 'Ms.' : 'Mr.';
                                $surname = trim((string) ($resignation->last_name ?? ''));
                                if ($surname === '') {
                                    $nameParts = preg_split('/\s+/', trim((string) ($resignation->name ?? '')));
                        ...
                        <p style="text-align: justify ;text-indent: 40px; line-height: 1.8; font: normal 13px, arial;">
                            ...
                            tendering your
                            resignation from the position of <u><b>{{ $resignation->position ?? ' ' }}</b></u> in
                            <u><b>{{ $resignation->division ?? ' ' }},</b></u> may I inform you that the same is hereby
                            accepted to
                            take effect on
                            <u><b>{{ date('F d, Y', strtotime($signatories['effectivity_date'] ?? $resignation->date_effectivity ?? '')) }}.</b></u>
                        </p>
                    </div>
                    <br>
                    {{-- <div>
                        <p style="text-align: justify ;text-indent: 40px; line-height: 1.8; font: normal 13px, arial;">
                            ...
                            <u>{{ $resignation->adjectival_rating ?? ' ' }}</u>
                            for your reference.
                        </p>
                    </div> --}}
                    <br>
                    ...
                    <div style="padding-top: 30px;float: right;">
                        ...
                        <p style="text-align: center; "><b>{{ $signatories['position1'] }}</b></p>
                    </div>
